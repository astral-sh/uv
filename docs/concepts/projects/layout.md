...
example, if a project directly depends on `fastapi`, the lockfile will also record the exact
versions of transitive dependencies such as `starlette` and `pydantic`. This file should be checked
