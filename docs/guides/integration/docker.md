...
- `ghcr.io/astral-sh/uv:{major}.{minor}.{patch}`, e.g., `ghcr.io/astral-sh/uv:0.7.1`
...
`ghcr.io/astral-sh/uv:{major}.{minor}-{base}`, e.g., `ghcr.io/astral-sh/uv:0.7.1-alpine`.
...
```dockerfile
ADD https://astral.sh/uv/0.7.1/install.sh /uv-installer.sh
```
...
!!! tip
    ...
    version tag, e.g., `ghcr.io/astral-sh/uv:0.7.1`, or (even better) the specific image digest,
