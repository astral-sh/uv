...
- `ghcr.io/astral-sh/uv:{major}.{minor}.{patch}`, e.g., `ghcr.io/astral-sh/uv:0.6.7`
...
`ghcr.io/astral-sh/uv:{major}.{minor}-{base}`, e.g., `ghcr.io/astral-sh/uv:0.6.7-alpine`.
...
```dockerfile
ADD https://astral.sh/uv/0.6.7/install.sh /uv-installer.sh
```
...
!!! tip
    ...
    version tag, e.g., `ghcr.io/astral-sh/uv:0.6.7`, or (even better) the specific image digest,
