...
- `ghcr.io/astral-sh/uv:{major}.{minor}.{patch}`, e.g., `ghcr.io/astral-sh/uv:0.4.23`
...
`ghcr.io/astral-sh/uv:{major}.{minor}-{base}`, e.g., `ghcr.io/astral-sh/uv:0.4.23-alpine`.
...
```dockerfile
COPY --from=ghcr.io/astral-sh/uv:0.4.23 /uv /uvx /bin/
```

Or, with the installer:

```dockerfile
ADD https://astral.sh/uv/0.4.23/install.sh /uv-installer.sh
```
