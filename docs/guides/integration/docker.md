...
- `uv:latest`
- `uv:{major}.{minor}.{patch}`, e.g., `uv:0.4.12`
...
`uv:{major}.{minor}.{patch}-{base}` and `uv:{major}.{minor}-{base}`, e.g., `uv:0.4.12-alpine`.
...
```dockerfile
ADD https://astral.sh/uv/0.4.12/install.sh /uv-installer.sh
```
