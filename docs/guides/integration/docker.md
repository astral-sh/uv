...
- `uv:latest`
- `uv:{major}.{minor}.{patch}`, e.g., `uv:0.4.14`
...
`uv:{major}.{minor}.{patch}-{base}` and `uv:{major}.{minor}-{base}`, e.g., `uv:0.4.14-alpine`.
...
```dockerfile
ADD https://astral.sh/uv/0.4.14/install.sh /uv-installer.sh
```
