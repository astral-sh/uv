...
- `ghcr.io/astral-sh/uv:{major}.{minor}.{patch}`, e.g., `ghcr.io/astral-sh/uv:0.10.10`
...
`ghcr.io/astral-sh/uv:{major}.{minor}-{base}`, e.g., `ghcr.io/astral-sh/uv:0.10.10-alpine`.
...
```dockerfile
ADD https://astral.sh/uv/0.10.10/install.sh /uv-installer.sh
```
