...
- `uv:latest`
- `uv:{major}.{minor}.{patch}`, e.g., `uv:0.4.5`
...
`uv:{major}.{minor}.{patch}-{base}` and `uv:{major}.{minor}-{base}`, e.g., `uv:0.4.5-alpine`.
...
```dockerfile
ADD https://astral.sh/uv/0.4.5/install.sh /uv-installer.sh
```
