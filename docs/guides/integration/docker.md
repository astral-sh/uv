...
- `ghcr.io/astral-sh/uv:{major}.{minor}.{patch}`, e.g., `ghcr.io/astral-sh/uv:0.5.6`
...
`ghcr.io/astral-sh/uv:{major}.{minor}-{base}`, e.g., `ghcr.io/astral-sh/uv:0.5.6-alpine`.
...
```dockerfile
ADD https://astral.sh/uv/0.5.6/install.sh /uv-installer.sh
```
