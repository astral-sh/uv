...
`if-necessary-or-explicit` instead. It prefers stable versions and permits pre-release fallback only
for first-party requirements that contain a pre-release identifier, while excluding pre-releases for
other packages.
