...
with tempfile.TemporaryDirectory() as tmpdir:
    script_path = Path(tmpdir) / "compare.sh"
    print(f"Creating script at `{script_path}`...", file=sys.stderr)
    with open(script_path, "wt") as script_file:
        script_file.write(output)

    print("Executing comparison...", file=sys.stderr)
