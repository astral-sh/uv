...
class PipSync(Suite):
    ...
    def install_cold(self, requirements_file: str, *, cwd: str) -> Command | None:
        ...
        return Command(
            ...
            prepare=f"rm -rf {cache_dir} && virtualenv --clear -p 3.11 {venv_dir}",
            command=[
                self.path,
                os.path.abspath(requirements_file),
                "--pip-args",
    ...
    def install_warm(self, requirements_file: str, *, cwd: str) -> Command | None:
        ...
        return Command(
            ...
            prepare=f"virtualenv --clear -p 3.11 {venv_dir}",
            command=[
                self.path,
                os.path.abspath(requirements_file),
                "--pip-args",
    ...
    def install_cold(self, requirements_file: str, *, cwd: str) -> Command | None:
        ...
        return Command(
            ...
            prepare=(
                ...
                f"rm -rf {cache_dir} && "
                f"rm -rf {data_dir} &&"
                f"virtualenv --clear -p 3.11 {venv_dir} --no-seed"
            ),
            command=[
                f"POETRY_CONFIG_DIR={config_dir}",
                f"POETRY_CACHE_DIR={cache_dir}",
...
class Pdm(Suite):
    ...
    def setup(self, requirements_file: str, *, cwd: str) -> None:
        ...
        subprocess.check_call(
            [self.path, "init", "--non-interactive", "--python", "3.11"],
            cwd=cwd,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
...
class UvProject(Suite):
    ...
    def install_warm(self, requirements_file: str, *, cwd: str) -> Command | None:
        ...
        return Command(
            name=f"{self.name} ({Benchmark.INSTALL_COLD.value})",
            prepare=(f"virtualenv --clear -p 3.11 {venv_dir} --no-seed"),
            command=[
                f"VIRTUAL_ENV={venv_dir}",
                self.path,
                "sync",
