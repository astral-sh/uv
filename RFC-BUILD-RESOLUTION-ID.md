...
resolutions whose package graph closures are not interchangeable. Build settings, extra build
dependencies, sources, indexes, constraints, and frontend policy remain freshness inputs; they do
not require an executor-specific namespace. Backend requirement metadata is assumed to be stable
...
- freshness validation must include build inputs that can change the selected closure without
  turning the interpreter that performed capture into serialized resolution identity.
...
2. How should the writer determine package-node compatibility without creating surprising lockfile
   churn?
3. Should scoped nodes duplicate artifact metadata, or should a later format factor artifacts out of
   package records?
4. How should `resolution-id` interact with package metadata exported to formats that do not
