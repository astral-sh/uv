...
RUN cargo zigbuild --bin axi --target $(cat rust_target.txt) --release
RUN cp target/$(cat rust_target.txt)/release/axi /axi
# TODO(konsti): Optimize binary size, with a version that also works when cross compiling
# RUN strip --strip-all /axi

FROM scratch
COPY --from=build /axi /axi
WORKDIR /io
ENTRYPOINT ["/axi"]
