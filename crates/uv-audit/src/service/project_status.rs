...
impl<'a> ProjectStatusAudit<'a> {
    ...
    pub async fn query_batch(&self, projects: &[(&PackageName, IndexUrl)]) -> Vec<Finding> {
        ...
        let semaphore = Arc::new(Semaphore::new(self.concurrency.downloads));
    ...
    async fn query_one(
        ...
        // Flat-index responses carry no project-status information.
        let archive = results.into_iter().find_map(|(_, format)| match format {
