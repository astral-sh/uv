...
impl DependencyScanner {
    ...
    async fn scan_from_lock(
        ...
        let lock_content = fs_err::tokio::read_to_string(lock_path)
            .await
            .map_err(|e| AuditError::Cache(e))?;

        let lock: Lock = toml::from_str(&lock_content).map_err(|e| AuditError::LockFileParse(e))?;
        ...
        .await
        .map_err(|e| AuditError::WorkspaceDiscovery(e))?;
        ...
        for package in lock.packages() {
            ...
            let dep_info = dependency_graph.get(&package_name).ok_or_else(|| {
                ...
                let available_packages: Vec<String> =
                    dependency_graph.keys().map(|k| k.to_string()).collect();
                debug!(
                    "Available packages in dependency graph: {:?}",
                    available_packages
                );
                AuditError::InvalidDependency(format!(
                    "Package {} not found in dependency graph. Available packages: [{}]",
                    package_name,
    ...
    fn get_direct_dependencies(&self, workspace: &Workspace) -> Result<Vec<PackageName>> {
        let mut direct_deps = Vec::new();

        for (_name, member) in workspace.packages() {
    ...
    fn extract_version_from_spec(&self, version_spec: &str) -> Option<Version> {
    ...
    fn determine_source_from_spec(
        ...
        if version_spec.contains("git+") || version_spec.contains(".git") {
            ...
            return Ok(DependencySource::Git { url, rev: None });
        }
    ...
    fn extract_path_from_spec(&self, version_spec: &str) -> Option<std::path::PathBuf> {
        if version_spec.contains("file://") {
    ...
    fn should_include_package_with_info(
        ...
        match dep_info.dependency_type {
            DependencyType::Dev => Ok(self.include_dev),
            DependencyType::Optional => Ok(self.include_optional),
            DependencyType::Main => Ok(true),
        }
    }
    ...
    fn build_dependency_graph(
        ...
        lock: &Lock,
        direct_deps: &[PackageName],
    ) -> Result<HashMap<PackageName, DependencyInfo>> {
        debug!(
    ...
    fn classify_dependency_type(&self, _package_name: &PackageName) -> DependencyType {
    ...
    fn determine_source_from_package(&self, package: &Package) -> Result<DependencySource> {
    ...
    fn extract_package_path(&self, _package: &Package) -> Option<std::path::PathBuf> {
    ...
    pub fn validate_dependencies(&self, dependencies: &[ScannedDependency]) -> Vec<String> {
        ...
        if placeholder_count > 0 {
            warnings.push(format!(
                "{} dependencies have placeholder versions. Run 'uv lock' for accurate version information.",
                placeholder_count
            ));
        }
        ...
        if non_registry_count > 0 {
            warnings.push(format!(
                "{} dependencies are from non-registry sources (git, path, URL). \
...
mod tests {
    ...
    fn test_version_extraction_from_spec() {
        ...
        // Test exact version
        let version = scanner.extract_version_from_spec("requests==2.31.0");
        assert_eq!(version, Some(Version::from_str("2.31.0").unwrap()));

        // Test minimum version
        let version = scanner.extract_version_from_spec("requests>=2.28.0");
        assert_eq!(version, Some(Version::from_str("2.28.0").unwrap()));

        // Test no version
        let version = scanner.extract_version_from_spec("requests");
        assert_eq!(version, None);

        // Test complex constraint
        let version = scanner.extract_version_from_spec("requests>=2.28.0,<3.0.0");
        assert_eq!(version, Some(Version::from_str("2.28.0").unwrap()));
    }
    ...
    fn test_source_detection_from_spec() {
        ...
        let source = scanner
            .determine_source_from_spec(&package_name, "requests>=2.31.0")
            .unwrap();
        assert!(matches!(source, DependencySource::Registry));

        // Test Git source
        let source = scanner
            .determine_source_from_spec(&package_name, "git+https://github.com/user/repo.git")
            .unwrap();
        assert!(matches!(source, DependencySource::Git { .. }));

        // Test path source
        let source = scanner
            .determine_source_from_spec(&package_name, "file:///path/to/package")
            .unwrap();
        assert!(matches!(source, DependencySource::Path));

        // Test URL source
        let source = scanner
            .determine_source_from_spec(&package_name, "https://example.com/package.tar.gz")
            .unwrap();
