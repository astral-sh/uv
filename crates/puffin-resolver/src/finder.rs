    ...
    fn select(
        ...
        for (version, files) in metadata.into_iter().rev() {
            ...
            for (filename, file) in files {
                ...
                // Use the most-compatible wheel
                if let DistFilename::WheelFilename(wheel) = filename.clone() {
                ...
                // Use the most-compatible sdist, if no wheel was found.
                if best_wheel.is_none() {
                    if let DistFilename::SourceDistFilename(sdist) = filename.clone() {
