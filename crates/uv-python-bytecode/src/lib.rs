...
"#;
        let expected = Command::new(python)
            .args([
                "-c",
                "import marshal, sys; code = compile(sys.stdin.read(), 'generic_closures.py', 'exec', dont_inherit=True, optimize=0); sys.stdout.buffer.write(marshal.dumps(code))",
            ])
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .spawn()
        ...
        assert!(expected.status.success());
        assert_eq!(
            compile(source, "generic_closures.py").unwrap().marshal(),
            expected.stdout
        );
    }
...
"#;
        let expected = Command::new(python)
            .args([
                "-c",
                "import marshal, sys; code = compile(sys.stdin.read(), 'annotation_scopes.py', 'exec', dont_inherit=True, optimize=0); sys.stdout.buffer.write(marshal.dumps(code))",
            ])
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .spawn()
        ...
        assert!(expected.status.success());
        assert_eq!(
            compile(source, "annotation_scopes.py").unwrap().marshal(),
            expected.stdout
        );
    }
...
"#;
    ...
    fn matches_cpython_marshal_for_a_terminal_finally_suite() {
        ...
        let expected = Command::new(python)
            .args([
                "-c",
                "import marshal, sys; code = compile(sys.stdin.read(), 'terminal_finally.py', 'exec', dont_inherit=True, optimize=0); sys.stdout.buffer.write(marshal.dumps(code))",
            ])
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .spawn()
        ...
        assert!(expected.status.success());
        assert_eq!(
            compile(source, "terminal_finally.py").unwrap().marshal(),
            expected.stdout
        );
    }
