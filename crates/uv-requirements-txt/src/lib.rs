...
/// Returns `true` if the given content appears to be a uv lockfile.
pub(crate) fn is_uv_lockfile(content: &str) -> bool {
    let mut has_version = false;
    let mut has_uv_lock_field = false;
    ...
    for line in content.lines() {
        ...
        if !has_version {
            ...
            // If the first non-comment non-empty line is not `version = <digits>`, it's not a uv lockfile.
            return false;
        }

        // Check for uv-lock specific fields
        if trimmed.starts_with("revision")
            || trimmed.starts_with("requires-python")
            || trimmed.starts_with("resolution-markers")
            || trimmed.starts_with("[options")
            || trimmed.starts_with("[manifest")
            || trimmed.starts_with("[conflicts")
            || trimmed.starts_with("supported-environments")
            || trimmed.starts_with("required-environments")
        {
            has_uv_lock_field = true;
            break;
        }

        // Check for `[[package]]` table headers, excluding Cargo.lock which has checksum / cargo markers
        if trimmed.starts_with("[[package]]")
            && !content.contains("checksum =")
            && !content.contains("@generated by Cargo")
        {
            has_uv_lock_field = true;
            break;
        }
...
pub enum RequirementsTxtParserError {
    ...
    /// The requirements file appears to be a uv lockfile.
    UvLockfile,
    #[cfg(feature = "http")]
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UvLockfile => {
                write!(
                    f,
                    "The file appears to be a uv lockfile, but uv lockfiles cannot be used as requirements files"
                )
            }
            #[cfg(feature = "http")]
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        match &self.error {
            ...
            RequirementsTxtParserError::UvLockfile => {
                write!(
                    f,
                    "The file `{file}` appears to be a uv lockfile, but uv lockfiles cannot be used as requirements files"
                )
            }
            #[cfg(feature = "http")]
...
mod test {
    ...
    use crate::{
        RequirementsTxt, RequirementsTxtParserError, calculate_row_column, is_uv_lockfile,
    };
    ...
    fn test_is_uv_lockfile() {
        // Standard uv lockfile
        assert!(is_uv_lockfile(indoc! {r#"
            version = 1
            revision = 1
        ...
        // Script lockfile without packages
        assert!(is_uv_lockfile(indoc! {r#"
            version = 1
            revision = 3
            requires-python = ">=3.12"
        ...
        // Lockfile with leading comments and whitespace
        assert!(is_uv_lockfile(indoc! {r#"
            # Generated by uv
        ...
        // Poetry lockfile must not match
        assert!(!is_uv_lockfile(indoc! {r#"
            [[package]]
            name = "certifi"
            version = "2024.2.2"
        ...
        // Normal requirements.txt must not match
        assert!(!is_uv_lockfile(indoc! {r"
            flask>=2.0
            requests==2.31.0
        "}));

        // pip-compile output must not match
        assert!(!is_uv_lockfile(indoc! {r"
    ...
    #[tokio::test]
    async fn uv_lockfile_error() -> Result<()> {
        let temp_dir = assert_fs::TempDir::new()?;
        let uv_lock = temp_dir.child("uv.lock");
        ...
        let err = RequirementsTxt::parse(uv_lock.path(), temp_dir.path())
            .await
            .unwrap_err();

        assert!(matches!(err.error, RequirementsTxtParserError::UvLockfile));
        assert!(err.to_string().contains("appears to be a uv lockfile"));
