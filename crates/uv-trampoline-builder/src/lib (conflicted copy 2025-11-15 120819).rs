...
"#
    ...
    fn sign_authenticode(bin_path: impl AsRef<Path>) {
        ...
        // Some developer machines may not have pwsh installed (e.g. using Windows PowerShell 5.1),
        // so we will skip signing in that case rather than fail the tests. The CI environment does
        // provide pwsh and will still exercise this code path.
        let pwsh = match which::which("pwsh") {
            Ok(path) => path,
            Err(_) => {
                println!("pwsh not found; skipping Authenticode signing in test");
                return;
            }
        };
        Command::new(pwsh)
