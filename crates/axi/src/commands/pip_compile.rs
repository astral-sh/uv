...
use axi_interpreter::{Interpreter, PythonVersion};
...
pub(crate) async fn pip_compile(
    ...
    let resolution = match resolver.resolve().await {
        Err(axi_resolver::ResolveError::NoSolution(err)) => {
    ...
    if include_header {
        writeln!(
            writer,
            "{}",
            format!("# This file was autogenerated by Axi v{VERSION} via the following command:")
                .green()
        )?;
        writeln!(
            writer,
