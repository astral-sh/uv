...
use uv_configuration::{
    ArtifactPolicy, BuildIsolation, ExcludeDependency, IndexStrategy, KeyringProviderType,
...
pub struct PipOptions {
    ...
    /// Control which distribution artifacts and hashes are included in output generated by
    /// `uv pip compile`.
    ///
    /// By default, all available artifacts are included. When build dependencies are included,
    /// source distributions are retained only when wheels do not cover the target environments.
    ///
    /// This option is experimental and may change without warning.
    #[option(
        default = "None",
