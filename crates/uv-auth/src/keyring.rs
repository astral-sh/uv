...
mod tests {
    ...
    async fn fetch_url_no_host() {
        ...
        let result =
            std::panic::AssertUnwindSafe(keyring.fetch(&LogSafeUrlRef::from(&url), Some("user")))
                .catch_unwind()
                .await;
    ...
    async fn fetch_url_with_password() {
        ...
        let result = std::panic::AssertUnwindSafe(
            keyring.fetch(&LogSafeUrlRef::from(&url), Some(url.username())),
        )
        .catch_unwind()
        .await;
        assert!(result.is_err());
    ...
    async fn fetch_url_no_auth() {
        let url = Url::parse("https://example.com").unwrap();
        let url = LogSafeUrlRef::from(&url);
        let keyring = KeyringProvider::empty();
    ...
    async fn fetch_url() {
        ...
        assert_eq!(
            keyring
                .fetch(&LogSafeUrlRef::from(&url), Some("user"))
                .await,
        ...
        assert_eq!(
            keyring
                .fetch(
                    &LogSafeUrlRef::from(&url.join("test").unwrap()),
                    Some("user")
                )
                .await,
            Some(Credentials::basic(
    ...
    async fn fetch_url_prefers_url_to_host() {
        ...
        assert_eq!(
            keyring
                .fetch(
                    &LogSafeUrlRef::from(&url.join("foo").unwrap()),
                    Some("user")
                )
                .await,
            Some(Credentials::basic(
        ...
        assert_eq!(
            keyring
                .fetch(
                    &LogSafeUrlRef::from(&url.join("bar").unwrap()),
                    Some("user")
                )
                .await,
            Some(Credentials::basic(
    ...
    async fn fetch_url_no_username() {
        let url = Url::parse("https://example.com").unwrap();
        let keyring = KeyringProvider::dummy([(url.host_str().unwrap(), "user", "password")]);
        let credentials = keyring.fetch(&LogSafeUrlRef::from(&url), None).await;
        assert_eq!(
            credentials,
            Some(Credentials::basic(
                Some("user".to_string()),
    ...
    async fn fetch_url_username_no_match() {
        ...
        let credentials = keyring.fetch(&LogSafeUrlRef::from(&url), Some("bar")).await;
        assert_eq!(credentials, None);
