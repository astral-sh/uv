...
impl Middleware for AuthMiddleware {
    ...
    async fn handle(
        ...
        let retry_request_url = LogSafeUrlRef::from(retry_request.url());
...
impl AuthMiddleware {
    ...
    async fn complete_request(
        ...
        let url = LogSafeUrl::from(request.url().clone());
    ...
    async fn complete_request_with_request_credentials(
            ...
            .get_url(request.url(), credentials.as_username().as_ref())
        {
            ...
            .fetch_credentials(
                Some(&credentials),
                LogSafeUrlRef::from(request.url()),
                index_url,
                auth_policy,
            )
            .await
    ...
    async fn fetch_credentials(
        ...
        url: LogSafeUrlRef<'_>,
        maybe_index_url: Option<&LogSafeUrl>,
        auth_policy: AuthPolicy,
            ...
            Some(ref keyring) => {
                ...
                if let Some(username) = credentials.and_then(|credentials| credentials.username()) {
                    if let Some(index_url) = maybe_index_url {
                        ...
                        keyring.fetch(&LogSafeUrlRef::from(index_url), Some(username)).await
                    } else {
                    ...
                    if let Some(index_url) = maybe_index_url {
                        ...
                        keyring.fetch(&LogSafeUrlRef::from(index_url), None).await
                    } else {
                        None
                    }
                } else {
...
fn tracing_url(request: &Request, credentials: Option<&Credentials>) -> LogSafeUrl {
    let mut url = LogSafeUrl::from(request.url().clone());
...
mod tests {
    ...
    -> Result<(), Error> {
        ...
        let indexes = Indexes::from_indexes(vec![
            Index {
                url: LogSafeUrl::from(base_url_1.clone()),
                root_url: LogSafeUrl::from(base_url_1.clone()),
                auth_policy: AuthPolicy::Auto,
            },
            Index {
                url: LogSafeUrl::from(base_url_2.clone()),
                root_url: LogSafeUrl::from(base_url_2.clone()),
                auth_policy: AuthPolicy::Auto,
            },
        ]);
    ...
    -> Result<(), Error> {
        ...
        let indexes = Indexes::from_indexes(vec![Index {
            url: LogSafeUrl::from(index_url.clone()),
            root_url: LogSafeUrl::from(index_url.clone()),
            auth_policy: AuthPolicy::Auto,
        }]);
    ...
    fn indexes_for(url: &Url, policy: AuthPolicy) -> Indexes {
        let mut url = LogSafeUrl::from(url.clone());
    ...
    fn test_tracing_url() {
        ...
        assert_eq!(
            ...
            LogSafeUrl::parse("https://pypi-proxy.fly.dev/basic-auth/simple").unwrap()
        );
        ...
        assert_eq!(
            ...
            LogSafeUrl::parse("https://user@pypi-proxy.fly.dev/basic-auth/simple").unwrap()
        );
        ...
        assert_eq!(
            tracing_url(&req, Some(&creds)),
            LogSafeUrl::parse("https://user:password@pypi-proxy.fly.dev/basic-auth/simple")
                .unwrap()
        );
    }
