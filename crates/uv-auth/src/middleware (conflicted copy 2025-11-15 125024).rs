...
impl Middleware for AuthMiddleware {
    ...
    async fn handle(
        ...
        // Determine the username for cache lookups from the request URL, falling back to the
        // provided `credentials` when present. Previously, we used `credentials` here which
        // resulted in incorrect cache use when the request URL contained a username but no
        // existing credentials were present.
        let username = if !retry_request_url.username().is_empty() {
            Username::from(Some(retry_request_url.username().to_string()))
        } else {
            credentials
                .as_ref()
...
impl AuthMiddleware {
    ...
    async fn complete_request_with_request_credentials(
        ...
        let maybe_cached_credentials = if let Some(index) = index {
            ...
            let realm = Realm::from(request.url());
            let provided_username = request.url().username();
            let specific = self
                .cache()
                .get_realm(realm.clone(), credentials.to_username());
            if specific.is_some() {
                specific
            } else if provided_username.is_empty() {
                // Only fall back to a username-agnostic realm credential when no username was
                // provided in the request URL. This ensures that a request containing an
                // explicit username does not accidentally receive a password for a different
                // username.
                self.cache().get_realm(realm, Username::none())
            } else {
                None
            }
        };
    ...
    async fn fetch_credentials(
        ...
        // Determine the username for this fetch from the request URL, not the existing
        // `credentials` object. Using the `credentials` parameter here results in incorrect
        // cache/index lookups when the request URL contains a username but no previous
        // credentials are present.
        let username = if !url.username().is_empty() {
            Username::from(Some(url.username().to_string()))
        } else if let Some(existing) = credentials {
            existing.to_username()
        } else {
            Username::none()
        };
        ...
        let key = if let Some(index) = index {
            (FetchUrl::Index(index.url.clone()), username.clone())
        } else {
            (FetchUrl::Realm(Realm::from(&**url)), username.clone())
        };
        ...
        let credentials = if let Some(credentials) = self.netrc.get().and_then(|netrc| {
            ...
            Credentials::from_netrc(
                netrc,
                url,
                // Use the username from the request URL if present.
                username.as_deref(),
            )
        }) {
        ...
        //      same realm we will use the wrong credentials.
            ...
            Some(ref keyring) => {
                ...
                // URLs; instead, we fetch if there's a username (from the request URL) or if the
                // user has requested to always authenticate.
            ...
            // If a keyring provider returned credentials, we should only use them when they
            // match the desired `username` for this fetch. The keyring backend may return a
            // credentials tuple with a username. If the request URL or provided `credentials`
            // requires a specific username, we must ensure the returned credential's username
            // matches, otherwise we risk authenticating a request for 'other_user' with a
            // different user's password (or vice-versa).
            if let Some(returned_username) = credentials.username() {
                // If the requested username is specified, ensure it matches the returned one.
                if let Some(requested) = username.as_deref() {
                    if requested != returned_username {
                        debug!(
                            "Keyring credentials username '{}' does not match requested '{}' for {url}, discarding",
                            returned_username,
                            requested,
                        );
                        None
                    ...
                    // No requested username, the returned credential can be used.
                    debug!("Found credentials in keyring for {url}");
                    Some(credentials)
                }
            } else {
                // If the returned credential has no username, it's safe to apply.
                debug!("Found credentials in keyring for {url}");
                Some(credentials)
            }
        } else {
