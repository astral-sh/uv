...
///
/// Deserialization of human input applies the same ambiguity checks as [`Self::parse`]. Protocol
/// messages and persisted URLs can use [`Self::deserialize_from_url`] to accept any valid URL.
///
/// # Examples
///
/// ```
...
impl DisplaySafeUrl {
    ...
    /// Deserialize a protocol or persisted URL without the human-input ambiguity heuristic.
    ///
    /// This accepts arbitrary valid URL paths, including `:` followed by `@`. Use ordinary
    /// [`Deserialize`] for human input that could contain unescaped credentials.
...
mod tests {
    ...
    fn deserialize_human_input() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for input in [
            "https://user/name:password@domain/a/b/c?sig=signature",
            "https://user\\name:password@domain/a/b/c?sig=sign@ature",
            "https://user#name:password@domain/a/b/c?sig=signature",
        ] {
            let deserializer =
                serde::de::value::StrDeserializer::<serde::de::value::Error>::new(input);
