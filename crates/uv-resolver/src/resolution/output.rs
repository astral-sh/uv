...
impl ResolverOutput {
    ...
    pub fn marker_tree(
        ...
        for i in self.graph.node_indices() {
            ...
            for req in self.constraints.apply(self.modifiers.apply_for(
                &dist.name,
                &dist.version,
        ...
        for direct_req in self
            .constraints
            .apply(self.modifiers.apply(self.requirements.iter()))
        {
...
fn has_lower_bound(
    ...
    for neighbor_index in graph.neighbors_directed(node_index, Direction::Incoming) {
        ...
        for requirement in modifiers
            ...
            .chain(modifiers.apply(metadata.dependency_groups.values().flatten()))
