        ...
        forks
            ...
            .filter(|result| {
                if let Ok(ref forked_state) = result {
                    ...
                    if crate::marker::is_definitively_empty_set(markers) {
                        return false;
                    }
                }
                true
...
impl Forks {
    ...
    fn add(&mut self, dep: PubGrubDependency) {
        use crate::marker::{is_definitively_empty_set, is_disjoint};
        ...
        let name = dep.package.name().expect("dependency always has a name");
        let mut markers = dep
            .package
            .marker()
            .cloned()
            .unwrap_or(MarkerTree::And(vec![]));
        if is_definitively_empty_set(&markers) {
        ...
        for mut fork in std::mem::take(&mut self.forks) {
            if is_disjoint(&fork.markers, &markers) {
                new.push(fork);
                continue;
            }
            found_overlap = true;
            ...
            if !is_definitively_empty_set(&not_markers) && !is_disjoint(&fork.markers, &not_markers)
            {
                new.push(Fork {
                    markers: {
                        let mut fork_markers = fork.markers.clone();
                        fork_markers.and(not_markers);
                        fork_markers
                    },
            ...
            if !is_definitively_empty_set(&markers) && !is_disjoint(&fork.markers, &markers) {
                ...
                new.push(Fork {
                    markers: {
                        let mut fork_markers = fork.markers;
                        fork_markers.and(markers);
                        fork_markers
                    },
        ...
        // I think ideally this would be truly guaranteed, since
        // it should always be the case that a non-trivial marker
        // expression ALWAYS overlaps with one of our forks, since the
        // markers on our forks should always cover the Universe. But
        // in practice, `is_disjoint` might not always produce the
        // correct result. So we could technically get here even if the
        // above logic is correct.
        //
        // Arguably we could therefore not assert and let this pass,
        // but it seems likely that this would lead to even more
        // confusing behavior. So for now, we bleat about this being a
        // bug.
        assert!(
            found_overlap,
