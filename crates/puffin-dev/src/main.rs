...
enum Cli {
    ...
    /// ```bash
    /// cargo run --bin axi-dev -- resolve-many scripts/popular_packages/pypi_8k_downloads.txt
    /// ```
    /// or
    /// ```bash
    /// cargo run --bin axi-dev -- resolve-many scripts/popular_packages/pypi_10k_most_dependents.txt
    /// ```
...
async fn main() -> ExitCode {
    ...
    let filter_layer = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::builder()
            ...
            .parse("axi_dev=info,axi_dispatch=info")
            .unwrap()
    });
    ...
    if let Err(err) = result {
        eprintln!("{}", "axi-dev failed".red().bold());
