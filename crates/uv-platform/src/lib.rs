...
impl Platform {
    ...
    /// Parse a platform from a cargo-dist style triple string.
    ///
    /// Format: `arch-vendor-os[-abi]`
    /// Examples: "aarch64-apple-darwin", "x86_64-unknown-linux-gnu", "x86_64-pc-windows-msvc"
    pub fn from_cargo_dist_triple(triple: &str) -> Result<Self, Error> {
        ...
        let libc_str = if parts.len() > 3 {
            // Handle special cases like gnueabihf -> gnu
            match parts[3] {
                s if s.starts_with("gnu") => "gnu",
                "musl" | "muslabi64" => "musl",
...
mod tests {
    ...
    fn test_cargo_dist_triple_roundtrip() {
        ...
        for original in platforms {
            ...
            assert_eq!(original.os.to_string(), parsed.os.to_string());
            // Architecture family should match
            assert_eq!(original.arch.family(), parsed.arch.family());
            assert_eq!(original.libc.to_string(), parsed.libc.to_string());
        }
    }
}
