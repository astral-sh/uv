...
use axi_traits::{BuildContext, BuildKind, SetupPyStrategy, SourceBuildTrait};
...
pub enum Error {
    ...
    Extraction(PathBuf, #[source] axi_extract::Error),
...
impl SourceBuild {
    ...
    pub async fn setup(
        ...
        let source_root = if metadata.is_dir() {
            ...
            axi_extract::archive(source, &extracted)
                .map_err(|err| Error::Extraction(extracted.clone(), err))?;

            // Extract the top-level directory from the archive.
            axi_extract::strip_component(&extracted)
                .map_err(|err| Error::Extraction(extracted.clone(), err))?
        };
    ...
    pub async fn build(&self, wheel_dir: &Path) -> Result<String, Error> {
        ...
        if let Some(pep517_backend) = &self.pep517_backend {
            // Prevent clashes from two axi processes building wheels in parallel.
