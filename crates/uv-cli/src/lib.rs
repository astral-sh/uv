...
pub struct PipCompileArgs {
    ...
    ///
    /// If multiple Python versions or platforms are specified, their exact-target resolutions are
    /// combined in this file, with markers for pins that apply to only some targets.
    ...
    /// provided. Otherwise, defaults to `requirements.txt`. Multiple exact Python targets
    /// can only be combined into a `requirements.txt` file.
    #[arg(long, value_enum)]
    ...
    ///
    /// By default, uv strips environment markers for a single target, as the resolution generated
    /// by `compile` is only guaranteed to be correct for the target environment. A multi-target
    /// resolution always retains the markers needed to distinguish its target environments.
    ...
    ///
    /// May be provided multiple times to combine separate exact-version resolutions in one output.
    #[arg(long, help_heading = "Python options")]
    pub python_version: Vec<PythonVersion>,
    ...
    ///
    /// May be provided multiple times to combine separate exact-platform resolutions in one output.
    #[arg(long)]
