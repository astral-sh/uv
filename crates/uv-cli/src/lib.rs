...
pub enum PythonCommand {
    ...
    /// python dir`.
    ///
    /// By default, Python executables are added to a directory on the path as `pythonX.y`. To
    /// install `python3` and `python`, use the `--default` flag. Use `uv python dir --bin` to
