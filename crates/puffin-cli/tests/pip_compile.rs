...
/// `pytest-fail-slow` uses `hatchling`, which supports `prepare_metadata_for_build_wheel()`. As
/// such, resolving the source distribution should not require building a wheel.
#[test]
fn prepare_metadata_for_build_wheeel() -> Result<()> {
    let temp_dir = TempDir::new()?;
    let cache_dir = TempDir::new()?;
    let venv = create_venv_py312(&temp_dir, &cache_dir);

    let requirements_in = temp_dir.child("requirements.in");
    requirements_in.write_str("pytest-fail-slow @ https://github.com/jwodder/pytest-fail-slow/releases/download/v0.4.0/pytest-fail-slow-0.4.0.tar.gz")?;
    ...
    insta::with_settings!({
        ...
        ----- stdout -----
        # This file was autogenerated by Puffin v0.0.1 via the following command:
        #    puffin pip-compile requirements.in --no-build --cache-dir [CACHE_DIR]
        iniconfig==2.0.0
            # via pytest
        packaging==23.2
            # via pytest
        pluggy==1.3.0
            # via pytest
        pytest==7.4.3
            # via pytest-fail-slow
        pytest-fail-slow @ https://github.com/jwodder/pytest-fail-slow/releases/download/v0.4.0/pytest-fail-slow-0.4.0.tar.gz
