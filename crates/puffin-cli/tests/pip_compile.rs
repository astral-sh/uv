...
/// Resolve a specific Flask source distribution via a Git SSH dependency.
#[test]
fn compile_git_ssh_dependency() -> Result<()> {
    let temp_dir = assert_fs::TempDir::new()?;
    let cache_dir = assert_fs::TempDir::new()?;
    let venv = temp_dir.child(".venv");
    ...
    let requirements_in = temp_dir.child("requirements.in");
    requirements_in.touch()?;
    requirements_in.write_str("flask @ git+ssh://git@github.com/pallets/flask.git")?;
