    ...
    fn try_from(wire: RequirementSourceWire) -> Result<Self, RequirementError> {
        match wire {
            ...
            RequirementSourceWire::Path { path } => {
                ...
                let ext = DistExtension::from_path(&path)
                    .map_err(|err| ParsedUrlError::MissingExtensionPath(path.to_path_buf(), err))?;
                Ok(Self::Path {
                    source: LocalSourcePath::new_preserving_absolute(path, url),
                    ext,
                })
            }
            RequirementSourceWire::Directory { directory } => {
                ...
                Ok(Self::Directory {
                    source: LocalSourcePath::new_preserving_absolute(directory, url),
                    editable: Some(false),
            ...
            RequirementSourceWire::Editable { editable } => {
                ...
                Ok(Self::Directory {
                    source: LocalSourcePath::new_preserving_absolute(editable, url),
                    editable: Some(true),
                    r#virtual: Some(false),
                })
            }
            RequirementSourceWire::Virtual { r#virtual } => {
                ...
                Ok(Self::Directory {
                    source: LocalSourcePath::new_preserving_absolute(r#virtual, url),
                    editable: Some(false),
                    r#virtual: Some(true),
                })
            }
