...
impl GitCheckout {
    ...
    fn reset(&self, with_lfs: bool) -> Result<()> {
        ...
        let ok_lfs_file = self.repo.path.join(LFS_READY_INDICATOR);
        let _ = paths::remove_file(&ok_file);
        if with_lfs {
            let _ = paths::remove_file(&ok_lfs_file);
        }
...
fn fetch_lfs(
    ...
    // needed to avoid issues with Git LFS filters not being configured on the
    // system and giving the wrong impression to the user that Git LFS objects
    // were initialized correctly when installation finishes.
