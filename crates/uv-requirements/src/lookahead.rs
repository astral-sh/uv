...
use distribution_types::{
    Dist, DistributionMetadata, LocalEditable, UvRequirement, UvRequirements, UvSource,
};
    ...
    async fn lookahead(&self, requirement: UvRequirement) -> Result<Option<RequestedRequirements>> {
        ...
        let dist = match requirement.source {
            ...
            UvSource::Url { url, subdirectory } => {
                ...
                // TODO(konsti): Don't drop given.
                Dist::from_https_url(requirement.name, VerbatimUrl::from_url(url))?
            }
            UvSource::Git { url, .. } => {
                // TODO(konsti): Don't drop given.
                Dist::from_git_url(requirement.name, url)?
            }
            UvSource::Path {
                path: _,
                url,
