...
// NOTE(glatinone): Normalize any Windows-style path separators when handling
// local source tree requirements and URL-derived requirements to ensure
// cross-platform equality and hashing behave consistently in lookahead. This
// avoids CI-only failures on Windows where backslashes would otherwise cause
// mismatches.
#[cfg(windows)]
fn normalize_sep(s: &str) -> std::borrow::Cow<'_, str> {
    if s.contains('\\') { std::borrow::Cow::Owned(s.replace('\\', "/")) } else { std::borrow::Cow::Borrowed(s) }
}
#[cfg(not(windows))]
fn normalize_sep(s: &str) -> std::borrow::Cow<'_, str> { std::borrow::Cow::Borrowed(s) }
...
impl<'a, Context: BuildContext> LookaheadResolver<'a, Context> {
    ...
    pub async fn resolve(
        ...
        while !queue.is_empty() || !futures.is_empty() {
            while let Some(mut requirement) = queue.pop_front() {
                // Normalize any file/URL path separators embedded in the requirement's source.
                if let RequirementSource::Url { url, .. } | RequirementSource::Path { url, .. } = &mut requirement.source {
                    let normalized = normalize_sep(url.as_str()).to_string();
                    *url = url::Url::parse(&normalized).unwrap_or_else(|_| url.clone());
                }
            ...
            while let Some(result) = futures.next().await {
                if let Some(mut lookahead) = result? {
                    ...
                    // Best-effort: ensure any URL carried forward has normalized separators on Windows.
                    if let Some(reqs) = lookahead.requirements_mut() {
                        for dep in reqs.iter_mut() {
                            if let RequirementSource::Url { url, .. } | RequirementSource::Path { url, .. } = &mut dep.source {
                                let normalized = normalize_sep(url.as_str()).to_string();
                                *url = url::Url::parse(&normalized).unwrap_or_else(|_| url.clone());
                            }
                        }
    ...
    async fn lookahead(
        ...
        let requires_dist = Box::into_iter(metadata.requires_dist)
            ...
            .map(|mut dependency| {
                // Normalize any embedded path separators in dependency sources.
                if let RequirementSource::Url { url, .. } | RequirementSource::Path { url, .. } = &mut dependency.source {
                    let normalized = normalize_sep(url.as_str()).to_string();
                    *url = url::Url::parse(&normalized).unwrap_or_else(|_| url.clone());
                }
