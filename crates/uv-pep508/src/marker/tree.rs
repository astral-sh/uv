        ...
        match self {
            ...
            MarkerExpression::String {
                ...
                if matches!(
                    ...
                    // Unwrap safety: The conditional above.
...
mod test {
    ...
    fn test_tilde_equal_normalization() {
        let marker1 = MarkerTree::from_str("python_version ~= '3.10.0'").unwrap();
        let marker2 =
            MarkerTree::from_str("python_version >= '3.10.0' and python_version < '3.11.0'")
                .unwrap();
        assert_eq!(marker1, marker2);

        // Two digit versions such as `python_version` get padded with a zero, so they can never
        // match
        let marker1 = MarkerTree::from_str("python_version ~= '3.10.1'").unwrap();
        let marker2 = MarkerTree::FALSE;
        assert_eq!(marker1, marker2);

        let marker1 = MarkerTree::from_str("python_version ~= '3.10'").unwrap();
        let marker2 =
            MarkerTree::from_str("python_version >= '3.10' and python_version < '4.0'").unwrap();
        assert_eq!(marker1, marker2);

        let marker1 = MarkerTree::from_str("python_full_version ~= '3.10.0'").unwrap();
        let marker2 = MarkerTree::from_str(
            "python_full_version >= '3.10.0' and python_full_version < '3.11.0'",
        )
        .unwrap();
        assert_eq!(marker1, marker2);

        let marker1 = MarkerTree::from_str("python_full_version ~= '3.10'").unwrap();
        let marker2 =
            MarkerTree::from_str("python_full_version >= '3.10' and python_full_version < '4.0'")
                .unwrap();
        assert_eq!(marker1, marker2);
    }
    ...
    fn test_tilde_equal_inverted_normalization() {
        let marker1 = MarkerTree::from_str("'3.0' ~= python_version").unwrap();
        let marker2 =
            MarkerTree::from_str("python_version >= '3' and python_version <= '3.0'").unwrap();
        assert_eq!(marker1, marker2);

        let marker1 = MarkerTree::from_str("'3.10' ~= python_version").unwrap();
        let marker2 =
            MarkerTree::from_str("python_version >= '3' and python_version <= '3.10'").unwrap();
        assert_eq!(marker1, marker2);

        let marker1 = MarkerTree::from_str("'3.10.0' ~= python_version").unwrap();
        let marker2 =
            MarkerTree::from_str("python_version >= '3' and python_version <= '3.10'").unwrap();
        assert_eq!(marker1, marker2);

        let marker1 = MarkerTree::from_str("'3.10' ~= python_full_version").unwrap();
        let marker2 = MarkerTree::from_str(
            "python_full_version < '3.11' and python_full_version >= '3.10.0'",
        )
        .unwrap();
        assert_eq!(marker1, marker2);

        let marker1 = MarkerTree::from_str("'3.10.10' ~= python_full_version").unwrap();
        let marker2 = MarkerTree::from_str(
            "python_full_version < '3.11' and python_full_version >= '3.10.0'",
        )
        .unwrap();
