...
impl SitePackages {
    ...
    pub fn satisfies(
            ...
            for requirement in requirements {
                match &requirement.requirement {
                    UnresolvedRequirement::Named(requirement) => {
                        named.push(NameRequirementSpecification::from(requirement.clone()));
                    }
                    UnresolvedRequirement::Unnamed(requirement) => {
                        match self.get_urls(requirement.url.verbatim.raw()).as_slice() {
                            ...
                            [distribution] => {
                                ...
                                named.push(NameRequirementSpecification::from(Requirement::from(
                                    requirement,
                                )));
                            }
                            _ => {
        ...
        self.satisfies_names(&requirements, constraints, &overrides, markers)
    }
        ...
        for entry in requirements {
            if let Some(r#overrides) = overrides.get(&entry.requirement.name) {
                ...
                if entry.requirement.evaluate_markers(Some(markers), &[]) {
                    if seen.insert(entry.requirement.clone()) {
                        stack.push(Cow::Borrowed(&entry.requirement));
                    }
                }
            }
        }
