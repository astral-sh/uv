...
impl Error {
    ...
    pub(crate) fn from_json_err(err: serde_json::Error, url: LogSafeUrl) -> Self {
        ErrorKind::BadJson { source: err, url }.into()
    }

    /// Create a new error from an HTML parsing error.
    pub(crate) fn from_html_err(err: html::Error, url: LogSafeUrl) -> Self {
...
pub enum ErrorKind {
    ...
    WrappedReqwestError(LogSafeUrl, #[source] WrappedReqwestError),
    ...
    AsyncHttpRangeReader(LogSafeUrl, #[source] AsyncHttpRangeReaderError),
    ...
    MissingContentType(LogSafeUrl),

    #[error("Invalid `Content-Type` header for {0}")]
    InvalidContentTypeHeader(LogSafeUrl, #[source] http::header::ToStrError),

    #[error("Unsupported `Content-Type` \"{1}\" for {0}. Expected JSON or HTML.")]
    UnsupportedMediaType(LogSafeUrl, String),
...
impl ErrorKind {
    pub(crate) fn from_reqwest(url: LogSafeUrl, error: reqwest::Error) -> Self {
