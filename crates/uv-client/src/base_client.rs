...
impl RetryableStrategy for UvRetryableStrategy {
    fn handle(&self, res: &Result<Response, reqwest_middleware::Error>) -> Option<Retryable> {
        let retryable = match res {
            ...
            // The uv retryable strategy handles the `default_on_request_failure` cases internally.
...
fn uv_retryable_strategy(err: &(dyn Error + 'static)) -> Option<Retryable> {
    ...
    while let Some(source) = current_source {
        // Handle different kinds of error nesting not accessible by downcast.
...
impl RetryState {
    ...
    pub async fn should_retry(&mut self, err: &(dyn Error + 'static), error_retries: u32) -> bool {
        ...
        match uv_retryable_strategy(err) {
    ...
    async fn retried_status_codes() -> Result<()> {
        ...
        for status in 100..599 {
            ...
            let uv_retry = match response.error_for_status() {
                Ok(_) => false,
                Err(err) => uv_retryable_strategy(&err) == Some(Retryable::Transient),
            };
