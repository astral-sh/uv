...
impl CachedClient {
    ...
    async fn fresh_request(
        ...
        // Check for retry information stored in response extensions before error_for_status
        ...
        if let Err(status_error) = response.error_for_status_ref() {
            if let Some(retries) = retry_count {
                // HTTP status error after retries - create custom error with retry information
            ...
            // HTTP status error without retries - use regular error
            return Err(ErrorKind::from_reqwest(url, status_error).into());
        }
