...
#[tokio::test]
async fn revalidation_preserves_payload_and_recovers_corruption() -> Result<()> {
    for corrupt in [false, true] {
        let server = MockServer::start().await;
        let url = format!("{}/metadata", server.uri()).parse()?;
        let client = CachedClient::new(BaseClientBuilder::default().build()?);
        let temp_dir = tempfile::tempdir()?;
        let cache_entry = CacheEntry::new(temp_dir.path().join("nested"), "cached");
        ...
        if corrupt {
            // Invalidate the MessagePack payload while retaining its HTTP cache policy.
            let mut bytes = fs_err::read(cache_entry.path())?;
            bytes[0] = 0xc1;
            fs_err::write(cache_entry.path(), bytes)?;
        ...
        for cache_control in [CacheControl::MustRevalidate, CacheControl::None] {
            let request = client.uncached().for_host(&url).get(url.as_str()).build()?;
            let result = client
                .get_serde_with_retry(request, &cache_entry, cache_control, async |response, _| {
                    response.text().await
                })
                .await
                .expect("cache revalidation should succeed");
            assert_eq!(result, if corrupt { "healed" } else { "cached" });
        }
        server.verify().await;
    }
    Ok(())
