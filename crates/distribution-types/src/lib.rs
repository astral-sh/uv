...
impl Dist {
    ...
    pub fn from_http_url(name: PackageName, url: VerbatimUrl) -> Result<Dist, Error> {
            ...
            .is_some_and(|ext| ext.eq_ignore_ascii_case("whl"))
        {
            ...
            DistKind::Built(BuiltDist::DirectUrl(DirectUrlBuiltDist { filename, url }))
        } else {
            DistKind::Source(SourceDist::DirectUrl(DirectUrlSourceDist { name, url }))
        };
...
#[cfg(test)]
mod test {
    use crate::{BuiltDist, DistKind, SourceDist};
    ...
    fn dist_size() {
        ...
        assert!(
            std::mem::size_of::<DistKind>() <= 248,
            "{}",
            std::mem::size_of::<DistKind>()
        );
        assert!(
