...
use distribution_types::{DirectUrlError, ParsedUrl, UvRequirement, UvSource};
use pep508_rs::{
    ...
    pub fn source(&self) -> UvSource {
        ...
        match self {
            ...
            Self::Unnamed(requirement) => {
                let direct_url = ParsedUrl::try_from(&requirement.url.to_url())
                    .expect("TODO(konsti): scheme not supported");
                UvSource::from_parsed_url(direct_url, requirement.url.clone())
            }
        }
    }
}
...
pub enum RequirementsTxtRequirementError {
    #[error(transparent)]
    ParsedUrl(#[from] Box<DirectUrlError>),
    #[error(transparent)]
