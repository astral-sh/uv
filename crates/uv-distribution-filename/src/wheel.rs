...
impl TryFrom<&Url> for WheelFilename {
    ...
    fn try_from(url: &Url) -> Result<Self, Self::Error> {
        ...
        // Strip query parameters from the filename, if they exist.
        let filename_without_query = if let Some(query_start) = filename.rfind('?') {
            &filename[..query_start]
        } else {
            filename
        };

        Self::from_str(filename_without_query)
    }
}
...
mod tests {
    ...
    #[test]
    fn ok_url_with_query_parameters() {
        let url_str = "https://foo.r2.cloudflarestorage.com/bar/pyfory-0.12.0.dev0-cp312-cp312-linux_aarch64.whl?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=foo%2Fauto%2Fs3%2Faws4_request&X-Amz-Date=20250623T043147Z&X-Amz-Expires=604800&X-Amz-SignedHeaders=host&X-Amz-Signature=bar";
        ...
        // Test with a URL that has no query parameters to ensure non-regression
        ...
        // The provided initial failing URL:
        ...
        assert_eq!(
            initial_failing_filename.to_string(),
            "pyfory-0.12.0.dev0-cp312-cp312-linux_aarch64.whl"
        );

        // Test with a URL that has no query parameters and ensure it still works.
        ...
        // Test with a URL where the path itself might contain something that looks like a whl file but isn't the last segment
