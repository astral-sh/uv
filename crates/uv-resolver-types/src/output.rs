        ...
        for edge in graph.edge_indices() {
            ...
            match (&graph[source], &graph[target]) {
                ...
                (
                    ...
                    let edge = if let Some(extra) = source_dist.variant.extra() {
                        Edge::Optional(extra.clone())
                    } else if let Some(group) = source_dist.variant.group() {
