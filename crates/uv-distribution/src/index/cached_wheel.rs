...
impl CachedWheel {
    ...
    pub fn from_http_pointer(path: impl AsRef<Path>, cache: &Cache) -> Option<Self> {
        ...
        // We expect the filename to be following same format as generated by `WheelFileName::stem_identifier`
        let filename = path.file_stem()?.to_str()?;
        if !looks_like_cache_digest(filename.rsplit_once('-')?.1) {
            return None;
        }
...
fn looks_like_cache_digest(s: &str) -> bool {
    s.bytes().len() == 16 && s.bytes().all(|c| matches!(c, b'a'..=b'f' | b'0'..=b'9'))
}
