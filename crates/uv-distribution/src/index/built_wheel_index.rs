use distribution_types::{DirectUrlSourceDist, git_reference, GitSourceDist, PathSourceDist};
...
use crate::source::{MANIFEST, read_http_manifest, read_timestamp_manifest};
...
impl BuiltWheelIndex {
    ...
    pub fn url(
        ...
        let cache_shard = cache.shard(
            ...
            WheelCache::Url(source_dist.url.raw()).bucket(),
        );
    ...
    pub fn path(
        ...
        let cache_shard = cache.shard(
            CacheBucket::BuiltWheels,
            WheelCache::Path(&source_dist.url).bucket(),
        );
