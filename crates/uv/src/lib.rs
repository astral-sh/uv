...
pub async fn run(cli: Cli, global_initialization: GlobalInitialization) -> Result<ExitStatus> {
    ...
    // Resolve quiet/verbose early so logging and warning collection respect `UV_QUIET` /
    // `UV_VERBOSE`. Command-line flags still win.
    let (quiet, verbose) = settings::resolve_quiet_verbose(
