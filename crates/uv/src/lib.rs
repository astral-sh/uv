...
async fn run(cli: Cli) -> Result<ExitStatus> {
    // Box the large command future to avoid stack overflows.
    Box::pin(run_with_global_initialization(cli, true)).await
}

#[cfg(feature = "benchmark")]
async fn run_benchmark(cli: Cli) -> Result<ExitStatus> {
    Box::pin(run_with_global_initialization(cli, false)).await
}

async fn run_with_global_initialization(cli: Cli, initialize_globals: bool) -> Result<ExitStatus> {
...
pub mod benchmark {
    ...
    pub async fn invoke(cli: Cli) -> Result<()> {
        ensure_success(run_benchmark(cli).await?)
    }

    fn ensure_success(status: ExitStatus) -> Result<()> {
        match status {
            ExitStatus::Success | ExitStatus::External(0) => Ok(()),
            ExitStatus::Failure => bail!("benchmark invocation failed"),
            ExitStatus::Error => bail!("benchmark invocation returned an unexpected error"),
            ExitStatus::External(code) => {
                bail!("benchmark invocation returned external exit status {code}")
            }
        }
    }
}
