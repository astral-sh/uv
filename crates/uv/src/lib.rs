...
#[instrument(skip_all)]
async fn run(cli: Cli, initialization: Initialization) -> Result<ExitStatus> {
    // Enable flag to pick up warnings generated by workspace loading.
    if initialization.enabled() && cli.top_level.global_args.quiet == 0 {
        uv_warnings::enable();
    }
    ...
    #[cfg(unix)]
    if initialization.enabled() && globals.preview.is_enabled(PreviewFeature::AdjustUlimit) {
    ...
    T: Into<OsString> + Clone,
{
    ...
    let main2 = move || {
        ...
        let result = runtime.block_on(Box::pin(run(cli, Initialization::Enabled)));
...
pub mod benchmark {
    ...
    use crate::{ExitStatus, Initialization, run};
