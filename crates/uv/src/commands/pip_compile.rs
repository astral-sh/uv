...
pub(crate) async fn pip_compile(
    ...
    if include_header {
        ...
        if !include_index_url {
            // Matches and captures --extra-index-url or --index-url along with their values,
            // accounting for both quoted and unquoted values, and removes any trailing whitespace.
            // SAFETY: The regex pattern is a constant string and is valid.
            let pattern = Regex::new(
                r#"(--extra-index-url|--index-url)[\s=]+("[^"\s]+"|'[^'\s]+'|[^\s]+)\s*"#,
            )
            .unwrap();
            arguments = pattern.replace_all(&arguments, "").trim().to_string();
        }
        writeln!(writer, "{}", format!("#    uv {arguments}").green())?;
    }
