...
use uv_python::request_from_version_file_in;
use uv_python::{
    requests_from_version_file_in, write_version_file, EnvironmentPreference, PythonInstallation,
    PythonPreference, PythonRequest, PYTHON_VERSION_FILENAME,
};
...
pub(crate) async fn pin(
    ...
    let virtual_project = if isolated {
        ...
        VirtualProject::discover(&std::env::current_dir()?, None)
            .await
            .inspect_err(|err| warn!("Failed to discover project: {err}"))
            .ok()
    };

    let target_dir = if let Some(virtual_project) = &virtual_project {
    ...
    let existing = request_from_version_file_in(&target_dir)
        .await
        .ok()
        .flatten();
    let target = target_dir.join(PYTHON_VERSION_FILENAME);
    write_version_file(&target, &output).await?;
