...
pub(crate) async fn add(
    ...
    .await
    {
        ...
        Err(err) => {
            ...
            match err {
                ProjectError::Operation(err) => diagnostics::OperationDiagnostic::with_tls_backend(client_builder.tls_backend()).with_hint(format!("If you want to add the package regardless of the failed resolution, provide the `{}` flag to skip locking and syncing.", "--frozen".green()))
                    .report(err)
