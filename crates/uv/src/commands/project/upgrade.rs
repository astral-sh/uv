...
async fn temporary_member_metadata<'a>(
        ...
        for declaration in current_project_dependency_declarations(project) {
            ...
            let declaration_contexts = contexts
                ...
                .map(|context| DeclarationContext {
                    ...
                    marker: requirement.marker.and(context.marker),
                })
                .collect::<Vec<_>>();
            ...
            for context in &declaration_contexts {
                let mut context_marker = context.marker.and(resolution_marker);
                ...
                effective_marker = effective_marker.or(context_marker);
...
fn collect_group_appearance_contexts(
    ...
    if let Some(dependency_groups) = &pyproject_toml.dependency_groups {
        for (parent, dependencies) in dependency_groups {
            ...
            let parent_marker = marker.and(dependency_group_requires_python_marker(
                pyproject_toml,
                parent,
            ));
            collect_group_appearance_contexts(
