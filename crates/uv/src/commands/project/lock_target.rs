...
use uv_cache::Cache;
use uv_configuration::{DependencyGroupsWithDefaults, DependencyModifier, NoSources};
...
impl<'lock> LockTarget<'lock> {
    ...
    pub(crate) fn overrides(
        self,
    ) -> Vec<DependencyModifier<uv_pep508::Requirement<VerbatimParsedUrl>>> {
        match self {
    ...
    pub(crate) fn exclude_dependencies(self) -> Vec<DependencyModifier<PackageName>> {
        match self {
