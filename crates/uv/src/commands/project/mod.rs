...
/// Return the environment if it is compatible, or if `--no-sync` permits incompatibility.
fn usable_project_environment(
    environment: PythonEnvironment,
    python_request: Option<&PythonRequest>,
    python_preference: PythonPreference,
    requires_python: Option<&RequiresPython>,
...
fn discover_project_environment(
    ...
    match PythonEnvironment::from_root(root, cache) {
        Ok(environment) => Ok(usable_project_environment(
            environment,
            python_request,
            python_preference,
            requires_python,
        ...
        Err(uv_python::Error::Query(uv_python::InterpreterError::NotFound(_))) => Ok(None),
