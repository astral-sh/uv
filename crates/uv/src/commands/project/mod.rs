...
pub(crate) enum ProjectError {
    ...
    #[error(
        "Package `{0}` was not found in project dependencies, but was specified for upgrade via `--upgrade-package`"
    )]
    MissingUpgradePackage(PackageName),

    #[error(
        "Group `{0}` was not found in the project's `dependency-groups` table, but was specified for upgrade via `--upgrade-group`"
    )]
