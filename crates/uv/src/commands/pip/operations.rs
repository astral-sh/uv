...
mod tests {
    ...
    fn classifies_distribution_failures() {
        assert!(uv_distribution::Error::NoBuild.is_user_failure());
        assert!(
    ...
    fn classifies_build_failures() {
        assert!(
            uv_types::AnyErrorBuild::from(uv_build_frontend::Error::InvalidBackendPath(
                "backend".to_string()
            ))
            .is_user_failure()
        );
        assert!(
            uv_types::AnyErrorBuild::from(uv_build_frontend::Error::BackendPathOutsideSourceTree(
                "../backend".to_string()
            ))
            .is_user_failure()
        );
        assert!(
            !uv_types::AnyErrorBuild::from(uv_build_frontend::Error::Io(std::io::Error::other(
                "disk failure"
            )))
            .is_user_failure()
        );
    ...
    #[test]
    fn build_type_erasure_retains_classification() {
        for error in [
            uv_distribution::Error::NoBuild,
            uv_distribution::Error::CacheRead(std::io::Error::other("broken cache")),
        ] {
            let expected = error.is_user_failure();
            let error = uv_types::AnyErrorBuild::from(error);
            assert_eq!(error.is_user_failure(), expected);

            let error = uv_types::AnyErrorBuild::from(
                uv_dispatch::BuildDispatchError::BuildFrontend(error),
            );
            assert_eq!(error.is_user_failure(), expected);
        }
    }
    ...
    fn classifies_git_io_failures() {
        assert!(
            !uv_git::GitResolverError::Io(std::io::Error::other("broken cache")).is_user_failure()
        );
        assert!(
            !uv_git::GitResolverError::Git(
