...
use uv_resolver::{
    AnnotationStyle, DependencyMode, DisplayResolutionGraph, ExactTargetOutput, ExcludeNewer,
    FlatIndex, ForkStrategy, InMemoryIndex, OptionsBuilder, Prerelease, PylockToml,
...
use crate::settings::PipCompileTarget;

/// A resolved target, retained until all targets succeed and the output can be written.
struct CompileResolution {
...
///
/// An explicit Python patch version selects `python_full_version`; otherwise, the
/// selector covers the target's entire major/minor series via `python_version`.
///
...
pub(crate) async fn pip_compile(
    ...
    compile_targets: Vec<PipCompileTarget>,
    ...
    let multiple_targets = !compile_targets.is_empty();
    if multiple_targets && universal {
        return Err(anyhow!(
    ...
    if multiple_targets && matches!(format, PipCompileFormat::PylockToml) {
        return Err(anyhow!(
    ...
    // Read the previous output once, before resolving any target or replacing the file.
    ...
    let build_options = build_options.combine(no_binary, no_build.clone());
    ...
    let targets = if multiple_targets {
        compile_targets
    } else {
        vec![PipCompileTarget {
            python_version,
            python_platform,
        }]
    };
    let mut resolutions: Vec<CompileResolution> = Vec::with_capacity(targets.len());
    for PipCompileTarget {
        mut python_version,
        python_platform,
    } in targets
    {
        ...
        let selector = if multiple_targets {
            let marker_env = resolver_env.marker_environment().ok_or_else(|| {
                anyhow!("Multiple Python targets require exact-platform resolution")
            })?;
        ...
        let relevant_markers =
            if include_marker_expression && matches!(format, PipCompileFormat::RequirementsTxt) {
                resolver_env
                    .marker_environment()
                    .map(|marker_env| {
                        resolution
        ...
        resolutions.push(CompileResolution {
            resolution,
            resolver_env,
            tags: tags.map(Cow::into_owned),
            selector,
            relevant_markers,
        });
    }
    ...
    let first = resolutions
        .first()
        .ok_or_else(|| anyhow!("Expected at least one Python target"))?;
    // Validate and format the entire output before writing anything, including stdout.
    let contents = match format {
        PipCompileFormat::RequirementsTxt => {
            let display = if multiple_targets {
                let outputs = resolutions
                    .iter()
                    ...
                    .collect::<Vec<_>>();
                DisplayResolutionGraph::from_targets(
                    &outputs,
                    &no_emit_packages,
                    generate_hashes,
                    include_extras,
    ...
    if matches!(format, PipCompileFormat::RequirementsTxt) {
        let relevant_markers = resolutions
            .iter()
    ...
    if !multiple_targets {
        operations::diagnose_resolution(first.resolution.diagnostics(), printer)?;
    }
