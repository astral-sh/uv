...
/// How progress updates are presented for the current reporter state.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum ProgressOutput {
    /// Rendered as progress bars through [`MultiProgress`].
    Rendered,
    /// Written as plain text because progress bars are unavailable.
    PlainText,
    /// Omitted to protect an SSH prompt or deterministic test output.
    Suppressed,
}
...
impl ProgressReporter {
    ...
    fn progress_output_mode(multi_progress: &MultiProgress, state: &BarState) -> ProgressOutput {
        ...
        } else if *HAS_UV_TEST_NO_CLI_PROGRESS {
            ProgressOutput::Suppressed
        } else {
            ProgressOutput::PlainText
        }
    }
    ...
    fn on_build_start(&self, source: &BuildableSource) -> usize {
        ...
        if Self::progress_output_mode(multi_progress, &state) == ProgressOutput::PlainText {
    ...
    fn on_build_complete(&self, source: &BuildableSource, id: usize) {
        ...
        let (progress, progress_output) = {
            ...
            (
                ...
                Self::progress_output_mode(multi_progress, &state),
            )
        };
        ...
        if progress_output == ProgressOutput::PlainText {
            let _ = writeln!(self.printer.stderr(), "{message}");
        }
...
mod tests {
    ...
    fn checkout_suspends_all_progress_draws() {
        ...
        reporter.root.tick();
        let download = reporter.on_download_start("example".to_string(), Some(10));
