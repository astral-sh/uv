...
/// Add command-specific resolution context and classify an operation error.
pub(crate) fn operation_error_with_context(
    context: &'static str,
    error: pip::operations::Error,
) -> UvError {
    match error {
        pip::operations::Error::Resolve(error) => UvError::from(pip::operations::Error::Resolve(
            error.with_resolution_context(context),
        )),
        pip::operations::Error::Requirements(error) => {
            UvError::user(requirements_error(context, error))
        }
        error => UvError::from(error),
    }
}
...
fn collect_operation_hints(cause: &(dyn std::error::Error + 'static), hints: &mut Hints<'static>) {
    ...
    if let Some(err) = cause.downcast_ref::<uv_resolver::ResolveError>() {
        match err {
            uv_resolver::ResolveError::Dist(_, dist, chain, error) => hints.extend(dist_hints(
                dist.name(),
                dist.version(),
                chain,
                error.hints(),
