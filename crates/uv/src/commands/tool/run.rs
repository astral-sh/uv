    ...
    let (from, environment) = match result {
        ...
        Err(ProjectError::Operation(err)) => {
            ...
            if from.is_none() && invocation_source == ToolRunCommand::Uvx && target == "run" {
                ...
                return Err(if user_error {
                    UvError::user(ToolRunUsageError {
                        cause: err,
            ...
            if let Some(verbose_flag) = find_verbose_flag(args) {
                let user_error = err.is_user_failure();
            ...
            return Err(match err {
                operations::Error::Resolve(err) => UvError::from(operations::Error::Resolve(
                    err.with_resolution_context("tool"),
                ))
                .into(),
                operations::Error::Requirements(err @ uv_requirements::Error::Dist(..)) => {
                    UvError::from(operations::Error::Requirements(err)).into()
                }
                operations::Error::Requirements(err) => {
                    UvError::user(diagnostics::requirements_error("tool", err)).into()
                }
        ...
        Err(ProjectError::Requirements(err)) => {
            return Err(UvError::user(diagnostics::requirements_error("`--with`", err)).into());
        }
