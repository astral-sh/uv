    ...
    let requires_python = match source.as_ref() {
        ...
        RequirementSource::Git {
            ...
            let client = match client_builder.build() {
                ...
                Err(err) => {
                    debug!(
                        "Failed to build client while inferring Python request (`{requirement}`): {err}"
                    );
                    return None;
                }
            };
...
mod tests {
    ...
    use super::read_requires_python_from_pyproject;
    ...
    fn infer_python_request_uses_full_specifier_range() {
        ...
        let requires_python = RequiresPython::from_specifiers(&requires_python);
        let request = uv_python::PythonRequest::from_requires_python(&requires_python).unwrap();
