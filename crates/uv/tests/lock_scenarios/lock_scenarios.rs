...
fn prerelease_marker_authorization_backtracks() -> Result<()> {
    ...
    cmd.env_remove(EnvVars::UV_EXCLUDE_NEWER);
    cmd.arg("--index-url").arg(server.index_url());
    // The rejected parent version's Linux requirements share pre-release admission, but the selected alternate parent retains only its plain requirement and selects stable `c==1.0.0`.
    uv_snapshot!(filters, cmd, @"
    success: true
    exit_code: 0
    ----- stdout -----
...
/// Requirements on the same marker proxy share pre-release admission within their fork.
///
/// ```text
...
fn prerelease_marker_equivalent_authorization() -> Result<()> {
    ...
    cmd.env_remove(EnvVars::UV_EXCLUDE_NEWER);
    cmd.arg("--index-url").arg(server.index_url());
    // The equivalent Linux requirements share pre-release admission and select `c==2.0.0a1` in the fork where they apply.
    uv_snapshot!(filters, cmd, @"
    success: true
    exit_code: 0
    ----- stdout -----
