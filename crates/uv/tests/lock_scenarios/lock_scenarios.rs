...
fn conflict_in_fork() -> Result<()> {
    ...
    error: No solution found when resolving dependencies for split (markers: sys_platform == 'os2'):
      Caused by: Because only b==1.0.0 is available and b==1.0.0 depends on d==1, we can conclude that all versions of b depend on d==1.
...
fn fork_non_local_fork_marker_direct() -> Result<()> {
    ...
    ----- stderr -----
    error: No solution found when resolving dependencies:
      Caused by: Because a==1.0.0 depends on c<2.0.0 and b==1.0.0 depends on c>=2.0.0, we can conclude that b==1.0.0 and a{sys_platform == 'linux'}==1.0.0 are incompatible.
...
fn fork_non_local_fork_marker_transitive() -> Result<()> {
    ...
    ----- stderr -----
    error: No solution found when resolving dependencies:
      Caused by: Because a==1.0.0 depends on c{sys_platform == 'linux'}<2.0.0 and b==1.0.0 depends on c{sys_platform == 'darwin'}>=2.0.0, we can conclude that a==1.0.0 and b==1.0.0 are incompatible.
