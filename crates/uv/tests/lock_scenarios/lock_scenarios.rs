...
/// This test checks upgrade outcomes for blocked fork rewrites alongside safe upgrades.
///
///
/// ```text
/// upgrade-outcomes
...
fn upgrade_outcomes() -> Result<()> {
    ...
    ----- stderr -----
    WARN Range requests not supported for foo-1.0.0-py3-none-any.whl; streaming wheel
    Resolved 4 packages in [TIME]
    "###
    );
