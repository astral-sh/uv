...
/// Test that environment variable credentials override URL-embedded credentials.
/// This is important for scenarios like Azure Artifacts where users might have
/// `VssSessionToken@` in the URL for keyring-based auth, but want to override
/// with explicit credentials in CI environments.
///
/// See: <https://github.com/astral-sh/uv/issues/...>
#[test]
fn env_credentials_override_url_credentials() -> Result<()> {
    let context = TestContext::new("3.12");
    ...
    pyproject_toml.write_str(
        ...
        [[tool.uv.index]]
        name = "internal"
        url = "https://wrong-user@pypi-proxy.fly.dev/basic-auth/simple/"
        explicit = true
        "#,
    )?;

    // First, verify that with wrong credentials in URL, it fails
    uv_snapshot!(context.filters(), context
        .pip_compile()
        .arg("pyproject.toml"), @r"
    success: false
    ...
    ----- stderr -----
      × No solution found when resolving dependencies:
      ╰─▶ Because iniconfig was not found in the package registry and project depends on iniconfig, we can conclude that your requirements are unsatisfiable.

          hint: An index URL (https://pypi-proxy.fly.dev/basic-auth/simple/) could not be queried due to a lack of valid authentication credentials (401 Unauthorized).
    ");

    // Now verify that environment variables override the URL credentials
    uv_snapshot!(context.filters(), context
        .pip_compile()
        .arg("pyproject.toml")
        .env("UV_INDEX_INTERNAL_USERNAME", "public")
