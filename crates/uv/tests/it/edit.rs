...
/// Test that `uv add` respects lockfile preferences for build dependencies.
#[test]
fn add_build_dependencies_respect_locked_versions() -> Result<()> {
    let context = TestContext::new("3.12").with_filtered_counts();
    ...
    // Create a lockfile with anyio 4.0.0
    uv_snapshot!(context.filters(), context.lock(), @r"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Ensure our build backend is checking the version correctly with a wrong version
    uv_snapshot!(context.filters(), context.add().arg("./child").env("EXPECTED_ANYIO_VERSION", "3.0"), @r"
    success: false
    exit_code: 1
    ----- stdout -----
    ...
    Resolved [N] packages in [TIME]
      ...
      ╰─▶ Call to `build_backend.build_editable` failed (exit status: 1)
          ...
          [stderr]
          Expected `anyio` version 3.0 but got 4.0.0
    ...
    uv_snapshot!(context.filters(), context.add().arg("./child").env("EXPECTED_ANYIO_VERSION", "4.0"), @r"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // Check that child was added correctly
    let pyproject_content = fs_err::read_to_string(pyproject_toml.path())?;
    assert!(pyproject_content.contains("dependencies = ["));
    assert!(pyproject_content.contains("child"));
    ...
    // The child should be rebuilt with anyio 3.7 on sync with reinstall
