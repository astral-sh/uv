...
use uv_cache_key::{GitRepositoryUrl, cache_digest};
...
fn add_git_lfs() -> Result<()> {
    ...
    uv_snapshot!(context.filters(), context.add()
        .arg("--no-cache")
        .arg("test-lfs-repo @ git+https://github.com/samypr100/test-lfs-repo")
    ...
    uv_snapshot!(context.filters(), context.add()
        .arg("--no-cache")
        .arg("git+https://github.com/samypr100/test-lfs-repo")
    ...
    ----- stderr -----
    Resolved 2 packages in [TIME]
      × Failed to download and build `test-lfs-repo @ git+https://github.com/samypr100/test-lfs-repo@657500f0703dc173ac5d68dfa1d7e8c985c84424#lfs=true`
    ...
    // Get the cache key
    let git_repo_url =
        GitRepositoryUrl::parse("https://github.com/samypr100/test-lfs-repo", Some(true))?;
    let repo_cache_key = cache_digest(&git_repo_url);

    // Remove the LFS objects
    let lfs_ok_checkout_file = git_checkouts
        .child(&repo_cache_key)
        .child("657500f")
        .child(".ok_lfs");
    let lfs_db_dir = git_db.child(&repo_cache_key).child(".git").child("lfs");

    fs_err::remove_file(&lfs_ok_checkout_file)?;
    fs_err::remove_dir_all(&lfs_db_dir)?;
    ...
    assert!(lfs_ok_checkout_file.exists());
    assert!(lfs_db_dir.exists());
