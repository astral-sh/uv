...
fn build_workspace_virtual_root() -> Result<()> {
    ...
    Building source distribution...
      × Failed to build `[TEMP_DIR]/`
      ╰─▶ [TEMP_DIR]/ appears to be a workspace root without a Python project, consider using `uv sync` or installing workspace members individually
    "###);
    Ok(())
}
...
fn build_pyproject_toml_not_a_project() -> Result<()> {
    ...
    Building source distribution...
      × Failed to build `[TEMP_DIR]/`
      ╰─▶ [TEMP_DIR]/ does not appear to be a Python project, as neither a `pyproject.toml` with a `[build-system]` table, nor a `setup.py`, nor a `setup.cfg` are present in the directory
    "###);
    Ok(())
}
