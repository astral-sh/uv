...
fn pep_751_infer_output_format() -> Result<()> {
    ...
    ----- stderr -----
    Resolved 4 packages in [TIME]
    error: `pyproject.toml` is reserved for project metadata; `uv export` exports produce requirements files or `pylock.toml` lockfiles. Choose a different output filename.
    ");
