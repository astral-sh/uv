...
/// Reinstalling a workspace tool without --editable converts editable deps to non-editable.
#[test]
fn tool_install_reinstall_converts_editable_to_noneditable() -> Result<()> {
    let context = TestContext::new("3.12").with_filtered_exe_suffix();
    let tool_dir = context.temp_dir.child("tools");
    let bin_dir = context.temp_dir.child("bin");

    // Workspace root project depending on a child workspace member.
    ...
    // Initial install in editable mode (simulating older editable-by-default behavior).
    uv_snapshot!(context.filters(), context.tool_install()
        .arg("--editable")
    ...
    // Verify the tool runs and reflects source.
    ...
    // Reinstall without --editable; should convert workspace deps to non-editable.
    let status = context
        .tool_install()
        .arg("--reinstall")
        .arg(context.temp_dir.path())
    ...
    // Modify the child source; since it's now non-editable, the tool should still print the old value.
    child_src
        .child("__init__.py")
...
/// Installing a workspace project as a tool should install workspace members
/// as non-editable so the tool remains usable if sources change.
#[test]
fn tool_install_no_editable_workspace_member() -> Result<()> {
    let context = TestContext::new("3.12").with_filtered_exe_suffix();
    let tool_dir = context.temp_dir.child("tools");
    let bin_dir = context.temp_dir.child("bin");
...
/// Installing a workspace project as a tool with --editable should install
/// workspace members in editable mode (coupled to source).
#[test]
fn tool_install_workspace_editable() -> Result<()> {
    let context = TestContext::new("3.12").with_filtered_exe_suffix();
    let tool_dir = context.temp_dir.child("tools");
    let bin_dir = context.temp_dir.child("bin");
