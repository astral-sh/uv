...
fn lock_invalid_exclude_newer_package_pyproject_errors() -> Result<()> {
    ...
    9 |         exclude-newer-package = { iniconfig = "invalid-date" }
      |                                               ^^^^^^^^^^^^^^
    failed to parse exclude-newer value: `invalid-date` could not be parsed as a valid exclude-newer value (expected a date like `2024-01-01`, a timestamp like `2024-01-01T00:00:00Z`, or a duration like `3 days` or `P3D`)
    "#);
