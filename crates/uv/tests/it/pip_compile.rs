...
/// Resolve with `--universal`, preserving marker-qualified workspace excludes.
#[test]
fn universal_marker_excludes() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    ...
    pyproject_toml.write_str(
        ...
        [tool.uv]
        exclude-dependencies = ["werkzeug ; sys_platform == 'win32'"]
        "#,
    )?;
    ...
    markupsafe==2.1.5
        ...
        #   jinja2
        #   werkzeug
    werkzeug==3.0.1 ; sys_platform != 'win32'
        # via flask
...
/// Resolve with the current interpreter, applying marker-qualified workspace excludes.
#[test]
fn marker_excludes() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    ...
    pyproject_toml.write_str(
        ...
        [tool.uv]
        exclude-dependencies = ["werkzeug ; python_version == '3.12'"]
        "#,
    )?;
...
/// Resolve with `--universal`, preserving marker-qualified excludes from a requirements source.
#[test]
fn universal_marker_excludes_from_file() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    ...
    excludes_txt.write_str("werkzeug ; sys_platform == 'win32'")?;
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --exclude excludes.txt --universal
    blinker==1.7.0
        # via flask
    click==8.1.7
        # via flask
