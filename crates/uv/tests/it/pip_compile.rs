...
fn unnamed_git_requirement() -> Result<()> {
    ...
    error: Git operation failed
      Caused by: failed to clone into: [CACHE_DIR]/git-v0/db/5a70990eb5793a40
      Caused by: failed to fetch branch or tag `3.0.0`
      Caused by: process didn't exit successfully: `/usr/bin/git fetch --force --update-head-ok 'https://github.com/pallets/flask.git' '+refs/tags/3.0.0:refs/remotes/origin/tags/3.0.0'` (exit status: 128)
    --- stderr
    error: RPC failed; curl 92 HTTP/2 stream 5 was not closed cleanly: CANCEL (err 8)
    error: 5192 bytes of body are still expected
