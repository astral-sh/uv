...
/// When `--exclude-newer` is used and an index doesn't provide `data-upload-time`,
/// files are excluded by default. With `--exclude-newer-last-modified`, a HEAD
/// request is made to the file URL to check the `Last-Modified` header.
#[tokio::test]
async fn compile_exclude_newer_last_modified() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let server = MockServer::start().await;

    // Download a real wheel to serve from the mock server.
    let wheel_bytes = reqwest::get("https://files.pythonhosted.org/packages/ef/a6/62565a6e1cf69e10f5727360368e451d4b7f58beeac6173dc9db836a5b46/iniconfig-2.0.0-py3-none-any.whl")
        .await
        .expect("failed to download wheel")
        .bytes()
        .await
        .expect("failed to read wheel bytes");

    // Serve a Simple API page WITHOUT `data-upload-time` attributes.
    let simple_page = format!(
        ...
        <html>
            ...
            <a href="{server_uri}/packages/iniconfig-2.0.0-py3-none-any.whl#sha256=2d91e135bf72d31a410b17c16da610a82cb55f6b0477d1a902134b24a455b8b3">iniconfig-2.0.0-py3-none-any.whl</a><br/>
        </body>
        </html>
        "#,
    ...
    Mock::given(method("GET"))
        .and(path("/iniconfig/"))
        .respond_with(ResponseTemplate::new(200).set_body_raw(simple_page.clone(), "text/html"))
        .mount(&server)
        .await;

    // Serve HEAD requests on the wheel file URL with a Last-Modified date of 2023-01-01.
    Mock::given(method("HEAD"))
        .and(path("/packages/iniconfig-2.0.0-py3-none-any.whl"))
        .respond_with(
    ...
    // Serve GET requests for the actual wheel file.
    Mock::given(method("GET"))
        .and(path("/packages/iniconfig-2.0.0-py3-none-any.whl"))
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_raw(wheel_bytes.to_vec(), "application/octet-stream"),
        )
        .mount(&server)
        .await;
    ...
    // Without `--exclude-newer-last-modified`, the file is excluded because it has
    // no `upload-time`, even though the cutoff is well after the actual upload date.
    uv_snapshot!(context
        .pip_compile()
        .env_remove(EnvVars::UV_EXCLUDE_NEWER)
        .arg("requirements.in")
    ...
    ----- stderr -----
    warning: iniconfig-2.0.0-py3-none-any.whl is missing an upload date, but user provided: 2024-01-01T00:00:00Z
      × No solution found when resolving dependencies:
      ╰─▶ Because there are no versions of iniconfig and you require iniconfig, we can conclude that your requirements are unsatisfiable.

          hint: `iniconfig` was filtered by `exclude-newer` to only include packages uploaded before 2024-01-01T00:00:00Z. Consider using `exclude-newer-package` to override the cutoff for this package.
    "
    );

    // With `--exclude-newer-last-modified`, the resolver makes a HEAD request and
    // sees `Last-Modified: Sun, 01 Jan 2023 00:00:00 GMT`, which is before the
    // cutoff of 2024-01-01, so the file is included.
    uv_snapshot!(context
        .pip_compile()
        .env_remove(EnvVars::UV_EXCLUDE_NEWER)
        .arg("requirements.in")
        ...
        .arg("--exclude-newer")
        .arg("2024-01-01T00:00:00Z")
        .arg("--exclude-newer-last-modified"), @"
    success: true
    exit_code: 0
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --exclude-newer 2024-01-01T00:00:00Z --exclude-newer-last-modified
    iniconfig==2.0.0
        # via -r requirements.in
...
/// When `--exclude-newer-last-modified` is used and the `Last-Modified` header indicates
/// the file is newer than the cutoff, the file is excluded and the resolver backtracks.
#[tokio::test]
async fn compile_exclude_newer_last_modified_too_new() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let server = MockServer::start().await;
    ...
    // Serve a Simple API page WITHOUT `data-upload-time`.
    let simple_page = format!(
        r#"
        <!DOCTYPE html>
        <html>
    ...
    Mock::given(method("GET"))
        .and(path("/iniconfig/"))
        .respond_with(ResponseTemplate::new(200).set_body_raw(simple_page, "text/html"))
        .mount(&server)
        .await;

    // Serve HEAD with a Last-Modified date that is AFTER the exclude-newer cutoff.
    Mock::given(method("HEAD"))
        .and(path("/packages/iniconfig-2.0.0-py3-none-any.whl"))
        .respond_with(
            ResponseTemplate::new(200)
                .insert_header("Last-Modified", "Mon, 01 Jul 2024 00:00:00 GMT"),
        )
        .mount(&server)
        .await;

    // Serve GET for the wheel (should never be reached).
    Mock::given(method("GET"))
        .and(path("/packages/iniconfig-2.0.0-py3-none-any.whl"))
        .respond_with(
            ResponseTemplate::new(200)
    ...
    // With `--exclude-newer-last-modified`, the HEAD request returns a date newer
    // than the cutoff, so the file is excluded.
    uv_snapshot!(context
        .pip_compile()
        .env_remove(EnvVars::UV_EXCLUDE_NEWER)
        .arg("requirements.in")
    ...
    ----- stderr -----
      × No solution found when resolving dependencies:
      ╰─▶ Because there are no versions of iniconfig and iniconfig==2.0.0 was excluded by --exclude-newer (via Last-Modified header), we can conclude that all versions of iniconfig cannot be used.
