...
fn compile_fallback_interpreter() -> Result<()> {
    ...
    // The short flag also allows complex requests, like alternative implementations, but we should
    // not fallback if that cannot be found
    uv_snapshot!(context.filters(), context.pip_compile()
        .arg("requirements.in")
        .arg("-p")
        .arg("pypy"), @r###"
...
fn compile_python_conflicts() -> Result<()> {
    ...
    // `--python-legacy` is not available in long-form
    uv_snapshot!(context.filters(), context.pip_compile()
            .arg("requirements.in")
            .arg("--python-legacy")
            .arg("3.12"), @r###"
    ...
    Usage: uv pip compile --cache-dir [CACHE_DIR] --python <PYTHON> <SRC_FILE>...
