...
fn markers_change_after_selection() -> Result<()> {
    ...
    ----- stderr -----
      ...
      ╰─▶ Because only a{sys_platform == 'win32'}==1.0.0 is available and a==1.0.0 has no Linux-compatible wheels, we can conclude that all versions of a{sys_platform == 'win32'} cannot be used.
