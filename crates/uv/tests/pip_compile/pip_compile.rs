...
/// PEP 723 constraints only narrow dependency edges that exist in the selected environment.
#[test]
fn scoped_constraint_dependency_from_script() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    context.temp_dir.child("script.py").write_str(r#"
# /// script
# requires-python = ">=3.12"
# dependencies = ["anyio==3.7.0"]
# [tool.uv]
# constraint-dependencies = [
#   { package = { name = "anyio" }, dependencies = ["idna==3.2", "typing-extensions==0", "sniffio==0; python_version < '3.12'"] },
# ]
# ///
    "#)?;
    uv_snapshot!(context.filters(), context.pip_compile().arg("script.py"), @"
    exit_code: 0 (success)
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] script.py
