...
fn assert_transitive_prerelease_reconsiders_stable_decision(requirement: &str) -> Result<()> {
    let context = uv_test::test_context!("3.12");
...
/// A transitive pre-release requirement should reconsider an earlier stable decision, even when
/// the stable version satisfies the requirement's range.
#[test]
fn transitive_prerelease_reconsiders_stable_decision() -> Result<()> {
    assert_transitive_prerelease_reconsiders_stable_decision("c>=1.0.0a1")
}

/// Late pre-release authorization through an extra should reconsider the base package decision.
#[test]
fn transitive_prerelease_extra_reconsiders_stable_decision() -> Result<()> {
    assert_transitive_prerelease_reconsiders_stable_decision("c[feature]>=1.0.0a1")
}

/// Late pre-release authorization through a marker should reconsider the base package decision.
#[test]
fn transitive_prerelease_marker_reconsiders_stable_decision() -> Result<()> {
    assert_transitive_prerelease_reconsiders_stable_decision("c>=1.0.0a1; python_version >= '3.12'")
}
...
/// Transitive pre-release requirements should compose across a dependency chain.
#[test]
fn transitive_prerelease_authorization_chain() -> Result<()> {
    let context = uv_test::test_context!("3.12");
...
/// Reusing cached dependency metadata should still reconsider an earlier stable decision when a
/// transitive pre-release requirement becomes active.
#[test]
fn transitive_prerelease_cached_dependencies_reconsider_stable_decision() -> Result<()> {
    let context = uv_test::test_context!("3.12");
...
#[test]
fn transitive_prerelease_suppresses_disabled_hint() -> Result<()> {
    let context = uv_test::test_context!("3.12");
