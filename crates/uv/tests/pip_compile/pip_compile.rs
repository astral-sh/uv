...
/// Preserve per-requirement build settings in compiled requirements files.
#[test]
fn editable_config_settings() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(&format!(
    ...
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in
    -e [WORKSPACE]/test/packages/setuptools_editable --config-settings=editable_mode=compat
        # via -r requirements.in
    iniconfig==2.0.0
        # via setuptools-editable
