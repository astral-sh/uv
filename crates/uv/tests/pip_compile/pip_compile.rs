...
/// Changing a build constraint must invalidate metadata produced by a legacy build backend.
#[test]
fn compile_invalidates_cached_legacy_build_metadata() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let links = context.workspace_root.join("test/links");

    let outer = context.temp_dir.child("outer");
    outer.create_dir_all()?;
    outer.child("pyproject.toml").write_str(indoc! {r#"
        [build-system]
        requires = ["ok"]
        backend-path = ["."]
    ...
    outer.child("build_backend.py").write_str(indoc! {r#"
        ...
        def build_wheel(wheel_directory, config_settings=None, metadata_directory=None):
            ...
            with ZipFile(Path(wheel_directory) / filename, "w") as wheel:
                ...
                wheel.writestr("outer-1.0.0.dist-info/METADATA", metadata())
                wheel.writestr(
    ...
    let build_a = context.temp_dir.child("build-a.txt");
    build_a.write_str("ok==1.0.0")?;
    let build_b = context.temp_dir.child("build-b.txt");
    build_b.write_str("ok==2.0.0")?;

    uv_snapshot!(context.filters(), context.pip_compile()
        .arg("outer/pyproject.toml")
        .arg("--no-index")
        .arg("--find-links")
        .arg(&links)
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] outer/pyproject.toml --no-index --build-constraint build-a.txt
    ok==1.0.0
        # via outer (outer/pyproject.toml)
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] outer/pyproject.toml --no-index --build-constraint build-b.txt
    ok==2.0.0
        # via outer (outer/pyproject.toml)
