...
fn emit_marker_expression_exciting_linux() -> Result<()> {
    let context = uv_test::test_context!("3.12.14");
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("anyio")?;
    ...
    #    python_full_version == '3.12.14' and platform_python_implementation == 'CPython' and sys_platform == 'linux'
    anyio==4.3.0
        # via -r requirements.in
    idna==3.6
        # via anyio
...
fn emit_marker_expression_pypy() -> Result<()> {
    ...
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --python-platform linux --emit-marker-expression
    # Pinned dependencies known to be valid for:
    #    python_full_version == '3.12.14' and implementation_name == 'cpython'
    pendulum==3.0.0
        # via -r requirements.in
