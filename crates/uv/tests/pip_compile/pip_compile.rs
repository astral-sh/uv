...
/// Require wheels only when the normally selected version has a compatible wheel.
#[test]
fn only_binary_if_available_preserves_selected_versions() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let scenario = toml::from_str::<Scenario>(indoc! {r#"
        name = "only-binary-if-available"
    ...
    // Strict wheels-only resolution selects the older source-only version with a wheel.
    uv_snapshot!(context.filters(), context.pip_compile()
        .arg("requirements.in")
        .arg("--index-url").arg(server.index_url())
        .arg("--only-binary").arg(":all:"), @"
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --only-binary :all:
    source-only==0.9.0
        # via -r requirements.in
    wheel-backed==1.0.0
        # via -r requirements.in
    ...
    // Conditional wheel selection preserves the latest version and permits its source fallback.
    uv_snapshot!(context.filters(), context.pip_compile()
        .arg("requirements.in")
        .arg("--index-url").arg(server.index_url())
        .arg("--only-binary").arg(":if-available:")
        .arg("--generate-hashes"), @r"
    exit_code: 0 (success)
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --only-binary :if-available: --generate-hashes
    source-only==1.0.0 \
    ...
    ----- stderr -----
    warning: The `--only-binary :if-available:` option is experimental and may change without warning. Pass `--preview-features only-binary-if-available` to disable this warning.
    Resolved 2 packages in [TIME]
    ");

    // Explicit package-specific source policies still override conditional wheel selection.
    uv_snapshot!(context.filters(), context.pip_compile()
        .arg("requirements.in")
        .arg("--index-url").arg(server.index_url())
        .arg("--only-binary").arg(":if-available:")
        .arg("--no-binary").arg("wheel-backed")
        .arg("--generate-hashes")
        .env(EnvVars::UV_PREVIEW_FEATURES, "only-binary-if-available"), @r"
    exit_code: 0 (success)
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --only-binary :if-available: --no-binary wheel-backed --generate-hashes
    source-only==1.0.0 \
        --hash=sha256:e7ead2a0decd519afce6959e902a8b0dec7e75fe611c183ef14139a36915a669
        # via -r requirements.in
    wheel-backed==1.0.0 \
        --hash=sha256:7b78e9a2ae9f21573caa8f2de56d24fba4b64f28121f69c8157f10dc03b15691
        # via -r requirements.in
    ...
    // The conditional policy can be emitted and parsed again by `uv pip install`.
    uv_snapshot!(context.filters(), context.pip_compile()
        .arg("requirements.in")
        .arg("--index-url").arg(server.index_url())
        .arg("--only-binary").arg(":if-available:")
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --only-binary :if-available: --emit-build-options --generate-hashes -o requirements.txt
    --only-binary :if-available:
...
/// Preserve source fallbacks until wheels cover every required universal environment.
#[cfg(feature = "test-universal")]
#[test]
fn only_binary_if_available_universal_required_environments() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let scenario = toml::from_str::<Scenario>(indoc! {r#"
        name = "only-binary-if-available-universal"
    ...
    context
        ...
        [tool.uv]
        preview-features = ["only-binary-if-available"]
...
/// Preserve artifact hashes when build-dependency inclusion is not combined with a binary policy.
#[test]
fn include_build_dependencies_preserves_artifact_hashes() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str("anyio==4.0.0")?;
...
/// Include build dependencies while applying the separately configured conditional wheel policy.
#[test]
fn include_build_dependencies_source_distribution() -> Result<()> {
    ...
    uv_snapshot!(context.filters(), context.pip_compile()
        ...
        .arg("--generate-hashes")
        .arg("-o").arg("requirements.txt")
        .arg("--preview-features").arg("include-build-dependencies,only-binary-if-available"), @r"
    exit_code: 0 (success)
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --include-build-dependencies --only-binary :if-available: --generate-hashes -o requirements.txt --preview-features include-build-dependencies,only-binary-if-available
    editables==0.5 \
...
fn include_build_dependencies_dynamic_hook_cached() -> Result<()> {
    ...
    context.temp_dir.child("uv.toml").write_str(indoc! {r#"
        preview-features = ["include-build-dependencies", "only-binary-if-available"]
