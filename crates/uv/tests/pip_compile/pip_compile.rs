...
#[test]
fn coalesce_prefetched_dependencies_multiple_indexes() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let secondary = PackseServer::new("backtracking/coalesce-prefetched-dependencies.toml");

    let mut core_versions = BTreeMap::new();
    for patch in 0..=30 {
        core_versions.insert(
            Version::from_str(&format!("3.0.{patch}"))?,
            PackageMetadata {
                requires: vec![Requirement::from_str("leaf==2.0.0")?],
                sdist: false,
                wheel: true,
                wheel_tags: vec![
                    "cp311-cp311-win_arm64"
                        .parse()
                        .expect("valid incompatible wheel tag"),
                ],
                ..PackageMetadata::default()
            },
        );
    ...
    let mut scenario = Scenario::empty();
    scenario.packages.insert(
        PackageName::from_str("core")?,
        Package {
            versions: core_versions,
        },
    );
    let primary = PackseServer::from_scenario(&scenario);
    ...
    uv_snapshot!(filters, context.pip_compile()
        .arg("requirements.in")
        .arg("--index-url")
        .arg(secondary.index_url())
        .arg("--extra-index-url")
        .arg(primary.index_url())
        .arg("--index-strategy")
        .arg("unsafe-best-match")
        .arg("-v"), @"
    success: true
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --index-strategy unsafe-best-match
    core==2.0.0
        # via parent
    leaf==1.0.0
        # via
...
/// Do not coalesce across a higher-priority index whose dependencies differ.
#[test]
fn coalesce_prefetched_dependencies_multiple_indexes_different_dependencies() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let secondary = PackseServer::new("backtracking/coalesce-prefetched-dependencies.toml");
    ...
    scenario.packages.insert(
        ...
        Package {
            versions: BTreeMap::from([(
                Version::from_str("3.0.12")?,
                PackageMetadata {
                    requires: vec![Requirement::from_str("leaf==1.0.0")?],
                    sdist: false,
                    wheel: true,
                    ..PackageMetadata::default()
                },
...
/// Coalesce matching, prefetched dependencies in a universal resolution that does not fork.
#[cfg(feature = "test-universal")]
#[test]
fn coalesce_prefetched_dependencies_universal() -> Result<()> {
    let context = uv_test::test_context!("3.12");
...
/// Do not coalesce across a prefetched wheel that requires a platform fork in a universal
/// resolution.
#[cfg(feature = "test-universal")]
#[test]
fn coalesce_prefetched_dependencies_universal_required_environment() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let secondary = PackseServer::new("backtracking/coalesce-prefetched-dependencies.toml");
                ...
                PackageMetadata {
                    ...
                    wheel_tags: vec![
                        "cp312-cp312-manylinux_2_17_x86_64"
                            .parse()
                            .expect("valid platform-limited wheel tag"),
                    ],
                    ..PackageMetadata::default()
                },
            )]),
    ...
    pyproject_toml.write_str(indoc! {r#"
        ...
        version = "0.1.0"
        requires-python = ">=3.12"
        dependencies = ["leaf==1.0.0", "parent"]
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] pyproject.toml --index-strategy unsafe-best-match --universal
    core==2.0.0
        # via parent
    leaf==1.0.0
        # via
