...
#[tokio::test]
async fn compile_fallback_interpreter_system_catalog() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    ...
    for catalog in [
        ...
        // System fallbacks do not need a catalog, even when managed installations are allowed.
        for preference in ["only-system", "system", "managed"] {
            allow_duplicates! {
                uv_snapshot!(context.filters(), context.pip_compile()
                    .arg("requirements.in")
                    .args(["--python-version", "3.8", "--no-python-downloads"])
                    .env_remove(EnvVars::VIRTUAL_ENV)
                    .env(EnvVars::UV_PYTHON_PREFERENCE, preference)
                    .env(EnvVars::UV_PYTHON_DOWNLOADS_JSON_URL, &catalog)
                ...
                uv_snapshot!(context.filters(), context.pip_compile()
                    .arg("requirements.in")
                    .args(["--python-version", "3.12.99", "--no-python-downloads"])
                    .env_remove(EnvVars::VIRTUAL_ENV)
                    .env(EnvVars::UV_PYTHON_PREFERENCE, preference)
                    .env(EnvVars::UV_PYTHON_DOWNLOADS_JSON_URL, &catalog)
                    .env(EnvVars::UV_HTTP_RETRIES, "0"), @"
                ...
                ----- stdout -----
                # This file was autogenerated by uv via the following command:
                #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --python-version 3.12.[X] --no-python-downloads

                ----- stderr -----
                warning: Requirements file `requirements.in` does not contain any dependencies
                warning: The requested Python version 3.12.[X] is not available; 3.12.[X] will be used to build dependencies instead.
                Resolved in [TIME]
                ");
            }
        }
