...
fn assert_prerelease_proxy_reconsiders_stable_decision(requirement: &str) -> Result<()> {
    let context = uv_test::test_context!("3.12");

    let package = |requires| -> Result<Package> {
        Ok(Package {
            versions: BTreeMap::from([(
    ...
    scenario.packages.insert(
        ...
        package(vec![Requirement::from_str("d")?])?,
    );
    scenario.packages.insert(
        ...
        Package {
            versions: BTreeMap::from([
                (
                    Version::from_str("1.0.0")?,
                    PackageMetadata {
                        extras: BTreeMap::from([(ExtraName::from_str("feature")?, Vec::new())]),
                        sdist: false,
                        wheel: true,
                        ..PackageMetadata::default()
                    },
    ...
    scenario.packages.insert(
        PackageName::from_str("d")?,
        package(vec![Requirement::from_str(requirement)?])?,
    );
    let server = PackseServer::from_scenario(&scenario);
    ...
    allow_duplicates! {
        ...
        ----- stdout -----
        # This file was autogenerated by uv via the following command:
        #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --prerelease=explicit
        a==1.0.0
            # via -r requirements.in
        b==1.0.0
            # via a
...
/// A pre-release proxy discovered after the wrapped package was decided should reconsider that
/// decision, even when the stable version also satisfies the proxy's range.
#[test]
fn prerelease_proxy_reconsiders_stable_decision() -> Result<()> {
    assert_prerelease_proxy_reconsiders_stable_decision("c>=1.0.0a1")
}

/// Late pre-release authorization through an extra should reconsider the base package decision.
#[test]
fn prerelease_extra_proxy_reconsiders_stable_decision() -> Result<()> {
    assert_prerelease_proxy_reconsiders_stable_decision("c[feature]>=1.0.0a1")
}

/// Late pre-release authorization through a marker should reconsider the base package decision.
#[test]
fn prerelease_marker_proxy_reconsiders_stable_decision() -> Result<()> {
    assert_prerelease_proxy_reconsiders_stable_decision("c>=1.0.0a1; python_version >= '3.12'")
}

fn stable_package_with_dependencies(requires: Vec<Requirement>) -> Result<Package> {
    Ok(Package {
        versions: BTreeMap::from([(
            Version::from_str("1.0.0")?,
            PackageMetadata {
...
fn package_with_prerelease(requires: Vec<Requirement>) -> Result<Package> {
    Ok(Package {
        versions: BTreeMap::from([
            (
                Version::from_str("1.0.0")?,
...
/// Deferred packages should remain visible as unresolved work so a chain of explicit
/// pre-release authorizations can be expanded from the end.
#[test]
fn prerelease_multiple_deferred_authorization_chain() -> Result<()> {
    let context = uv_test::test_context!("3.12");

    let mut scenario = Scenario::empty();
    scenario.packages.insert(
        PackageName::from_str("a")?,
        stable_package_with_dependencies(vec![
            Requirement::from_str("x>1")?,
            Requirement::from_str("y>1")?,
            Requirement::from_str("b>1")?,
            Requirement::from_str("z")?,
        ])?,
    );
    scenario.packages.insert(
        PackageName::from_str("b")?,
        package_with_prerelease(vec![Requirement::from_str("y>=2.0.0a1")?])?,
    );
    scenario.packages.insert(
        PackageName::from_str("x")?,
        package_with_prerelease(Vec::new())?,
    );
    scenario.packages.insert(
        PackageName::from_str("y")?,
        package_with_prerelease(vec![Requirement::from_str("x>=2.0.0a1")?])?,
    );
    scenario.packages.insert(
        PackageName::from_str("z")?,
        stable_package_with_dependencies(vec![Requirement::from_str("b>=2.0.0a1")?])?,
    );
    let server = PackseServer::from_scenario(&scenario);
...
/// Retrying multiple deferred packages should terminate when none receives authorization.
#[test]
fn prerelease_multiple_deferred_without_authorization() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    ...
    ----- stderr -----
      ...
      ╰─▶ Because only y<=1 is available and a==1.0.0 depends on y>1, we can conclude that a==1.0.0 cannot be used.
          And because only a==1.0.0 is available and you require a, we can conclude that your requirements are unsatisfiable.

    hint: Pre-releases are available for `y` in the requested range (e.g., 2.0.0a1), but pre-releases weren't enabled (try: `--prerelease=allow`)
    ");

    Ok(())
}

/// Reactivating cached dependency incompatibilities should reconsider stable decisions that were
/// made while their pre-release proxies were inactive.
#[test]
fn prerelease_cached_proxy_reconsiders_stable_decision() -> Result<()> {
    let context = uv_test::test_context!("3.12");
        ...
        Package {
            versions: BTreeMap::from([
                ...
                (
                    Version::from_str("2.0.0")?,
                    PackageMetadata {
                        requires: vec![Requirement::from_str("p")?, Requirement::from_str("x>1")?],
                        sdist: false,
                        wheel: true,
                        ..PackageMetadata::default()
                    },
    ...
    scenario.packages.insert(
        PackageName::from_str("p")?,
        stable_package_with_dependencies(vec![Requirement::from_str("c>=1.0.0a1")?])?,
    );
    scenario.packages.insert(
        PackageName::from_str("c")?,
        package_with_prerelease(Vec::new())?,
    ...
    scenario.packages.insert(
        PackageName::from_str("d")?,
        stable_package_with_dependencies(vec![Requirement::from_str("p")?])?,
    );
    scenario.packages.insert(
        PackageName::from_str("x")?,
        stable_package_with_dependencies(Vec::new())?,
    );
    let server = PackseServer::from_scenario(&scenario);
...
/// A no-candidate result caused by inactive pre-release authorization should not prevent an older
/// parent version from activating the required proxy.
#[test]
fn prerelease_no_versions_is_scoped_to_active_decisions() -> Result<()> {
    let context = uv_test::test_context!("3.12");
        ...
        Package {
            versions: BTreeMap::from([
                (
                    Version::from_str("1.0.0")?,
                    PackageMetadata {
                        requires: vec![Requirement::from_str("c>=2.0.0a1")?],
                        sdist: false,
                        wheel: true,
                        ..PackageMetadata::default()
                    },
                ...
                (
                    Version::from_str("2.0.0")?,
                    PackageMetadata {
                        requires: vec![Requirement::from_str("c>1")?],
                        sdist: false,
                        wheel: true,
                        ..PackageMetadata::default()
                    },
