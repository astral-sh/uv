...
/// Pre-release proxies should inherit an ordinary constraint on the wrapped package before
/// selecting a version, rather than rejecting each out-of-range pre-release individually.
#[test]
fn prerelease_proxy_respects_base_range() -> Result<()> {
    let context = uv_test::test_context!("3.12");

    let mut prerelease_versions = (1..=200)
        .map(|version| {
            Ok((
                Version::from_str(&format!("2.0.0a{version}"))?,
                PackageMetadata {
                    sdist: false,
                    wheel: true,
                    ..PackageMetadata::default()
    ...
    scenario.packages.insert(
        ...
        Package {
            versions: BTreeMap::from([(
                Version::from_str("1.0.0")?,
                PackageMetadata {
                    requires: vec![Requirement::from_str("c>=1.0.0a1")?],
                    sdist: false,
                    wheel: true,
                    ..PackageMetadata::default()
                },
    ...
    let output = uv_snapshot!(filters, command, @"
    success: true
    exit_code: 0
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    ...
    let proxy_decisions = stderr
        .lines()
        .filter(|line| line.contains("Adding transitive dependency for c=="))
        .count();
    assert_debug_snapshot!(proxy_decisions, @"1");
