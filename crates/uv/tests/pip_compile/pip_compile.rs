...
/// A package needed only by a rejected version may be absent from the solution.
#[test]
fn pubgrub_absent_dependency_after_backtracking() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let scenario = toml::from_str::<Scenario>(indoc! {r#"
        name = "absent-dependency-after-backtracking"
...
/// Backtracking several decisions must restore the earlier version constraints.
#[test]
fn pubgrub_restore_constraints_after_backtracking() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let scenario = toml::from_str::<Scenario>(indoc! {r#"
        name = "restore-constraints-after-backtracking"

        [root]
        requires = ["retained==1.0.0", "a"]

        [expected]
        satisfiable = true

        [packages.retained.versions."1.0.0"]
