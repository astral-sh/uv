...
/// Regression test for <https://github.com/astral-sh/uv/issues/3750>, where we lose the verbatim
/// representation of the three url in the warm cache case.
#[test]
fn compile_transitive_pyproject_toml_verbatim_url() -> Result<()> {
    let context = TestContext::new("3.12");
    let one = context.temp_dir.child("pyproject.toml");
    one.write_str(indoc! {r#"
        [project]
        name = "one"
        version = "0.0.1"
        dependencies = ["two @ file://${PROJECT_ROOT}/two"]
    "#})?;
    let two = context.temp_dir.child("two/pyproject.toml");
    two.write_str(indoc! {r#"
        [project]
        name = "two"
        version = "0.0.1"
        dependencies = ["three @ file://${PROJECT_ROOT}/three"]
    "#})?;
    let three = context.temp_dir.child("three/pyproject.toml");
    three.write_str(indoc! {r#"
        [project]
        name = "three"
        version = "0.0.1"
        dependencies = []
