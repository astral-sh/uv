...
/// Request `hatchling_editable`, which depends on `git+https://github.com/pytest-dev/iniconfig@9cae43103df70bac6fde7b9f35ad11a9f1be0cb4`.
#[test]
#[cfg(feature = "git")]
...
/// Request `hatchling_editable`, which depends on `git+https://github.com/pytest-dev/iniconfig@9cae43103df70bac6fde7b9f35ad11a9f1be0cb4`.
/// Since we also requested `iniconfig` at a different URL, we should reject it.
#[test]
#[cfg(feature = "git")]
...
fn pre_release_path_requirement() -> Result<()> {
    ...
    let source_tree = TempDir::new()?;
    let pyproject_toml = source_tree.child("pyproject.toml");
    pyproject_toml.write_str(
        r#"[project]
name = "example"
version = "0.0.0"
...
"#,
    ...
    // Write to a requirements file.
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(&format!("{}", source_tree.path().display()))?;

    let filters: Vec<_> = [("example @ .*", "example @ [SOURCE_TREE]")]
        .into_iter()
        .chain(INSTA_FILTERS.to_vec())
        .collect();
...
"#,
    ...
    // Write to a requirements file.
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(&format!("-e {}", source_tree.path().display()))?;

    let filters: Vec<_> = [("-e .*", "-e [SOURCE_TREE]")]
        .into_iter()
        .chain(INSTA_FILTERS.to_vec())
        .collect();
