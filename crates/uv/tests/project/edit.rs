...
async fn add_auth_policy_never_with_url_credentials_ignored() -> Result<()> {
    ...
    error: Failed to add dependencies
      Caused by: No solution found when resolving dependencies:
      Caused by: Because only anyio==4.3.0 is available and anyio==4.3.0 could not be fetched from the network (`401 Unauthorized`), we can conclude that all versions of anyio cannot be used.
