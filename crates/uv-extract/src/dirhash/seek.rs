...
mod tests {
    ...
    use super::{unzip, unzip_and_hash};
    ...
    #[test]
    fn seekable_unzip_without_digest_extracts_files() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let files = unzip(fs_err::File::open(&archive_path)?, &extract)?;

        assert_eq!(files, [(PathBuf::from("example/data.txt"), 8)]);
        assert_eq!(fs_err::read(extract.join("example/data.txt"))?, b"contents");
        Ok(())
    }

    #[test]
    fn streaming_unzip_without_digest_extracts_files() -> Result<(), Box<dyn std::error::Error>> {
        let entries = [ZipEntry {
            path: "example/data.txt",
            contents: b"contents",
            mode: 0o100_644,
        ...
        let files = stream_unzip(&archive_path, &extract)?;
    ...
    #[test]
    fn extractors_reject_duplicate_output_paths() -> Result<(), Box<dyn std::error::Error>> {
        let first_entries = [
            ZipEntry {
                path: "example/data.txt",
                contents: b"first",
        ...
        for (name, entries) in [
            ...
            ("identical", identical_entries.as_slice()),
            ...
            let stream_extract = temp_dir.path().join(format!("{name}-stream"));
            fs_err::create_dir_all(&stream_extract)?;
            let result = stream_unzip_and_hash(&archive_path, &stream_extract);
    ...
    fn stream_unzip_and_hash(
        archive: &Path,
        target: &Path,
    ) -> Result<(usize, DirectoryDigest), crate::Error> {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .map_err(crate::Error::Io)?;
        runtime.block_on(async {
            let file = fs_err::tokio::File::open(archive)
                .await
                .map_err(crate::Error::Io)?;
            let (files, digest) =
    ...
    fn stream_unzip(
        ...
        runtime.block_on(async {
            ...
            crate::stream::unzip(archive.display(), file, target)
                .await
                .map_err(Into::into)
        })
    }
